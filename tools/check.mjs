...
const SUITES = [
  ...
  // 幽灵作业：卡住的管线必须能被看见、被停止、被放行并留下日志
  // （用户实测："已有一条管线在运行" + 没有日志 + 底下什么都没跑）。
  ['幽灵作业/看门狗', 'manju-watchdog-test.mjs', null],
  ...
  // 小说工作区（D:\Ai\小说）与一键做视频：作品扫描 / 详情 / 正文 / 建项目 + 写溯源 + 排集号。
  // 夹具自带（临时工作区），并且会把小说工作区目录恢复原样 —— 不碰用户真实的小说。
  ['小说工作区/一键做视频', 'manju-novel-test.mjs', null],
  // 宿主合成路径（界面「合成」按钮走的那条）：以前只有源码文本断言，
  // 这条套件真造 2 镜小项目、真调一次 assemble，再逐条量产出的 ASS。
  ['合成契约（宿主）', 'manju-compose-test.mjs', null],
