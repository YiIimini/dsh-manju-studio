...
.mj-root ::-webkit-scrollbar{width:9px;height:9px}
.mj-root ::-webkit-scrollbar-track{background:transparent}
.mj-root ::-webkit-scrollbar-thumb{background:color-mix(in srgb,var(--mj-dim) 30%,transparent);border-radius:var(--mj-r-cap);border:2px solid transparent;background-clip:content-box}
.mj-root ::-webkit-scrollbar-thumb:hover{background:color-mix(in srgb,var(--mj-dim) 55%,transparent);background-clip:content-box}
...
.mj-products>.mj-p{flex:1 1 auto;max-height:30vh}
...
.mj-ph{display:flex;align-items:center;gap:8px;padding:9px 12px;flex:0 0 auto;font-size:var(--mj-fs-micro);font-weight:700;border-bottom:1px solid var(--mj-line);background:linear-gradient(180deg,color-mix(in srgb,var(--mj-text) 5%,transparent),transparent)}
...
.mj-boardgrid{flex:1 1 auto;min-height:0;overflow:auto;display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:11px;align-content:start;padding:1px}
...
.mj-prow{position:relative;display:flex;align-items:center;gap:8px;padding:6px 7px;border:1px solid transparent;border-radius:var(--mj-r-sm);cursor:pointer;min-width:0;transition:background var(--mj-t),border-color var(--mj-t)}
...
.mj-pthumb{flex:0 0 auto;width:56px;height:32px;border-radius:var(--mj-r-xs);object-fit:cover;background:var(--mj-surface2);outline:1px solid rgba(255,255,255,.09);outline-offset:-1px}
...
.mj-pinfo{flex:1 1 auto;min-width:0;display:flex;flex-direction:column;gap:1px}
.mj-ptitle{font-size:var(--mj-fs-body);font-weight:600;line-height:1.3;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.mj-pmeta{font-size:var(--mj-fs-micro);color:var(--mj-dim);line-height:1.25;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-variant-numeric:tabular-nums}
.mj-pbar{height:3px;border-radius:1px;background:var(--mj-line);overflow:hidden;margin-top:5px}
.mj-pbar i{display:block;height:100%;border-radius:1px;background:var(--mj-dim);transition:width var(--mj-t)}
.mj-pbar.done i{background:var(--mj-ok)}
.mj-pfinal{flex:0 0 auto;color:var(--mj-ok);font-size:13px;font-weight:700;line-height:1}
...
.mj-coverrename{position:absolute;left:9px;right:9px;bottom:38px;width:auto;background:rgba(0,0,0,.8);border:1px solid var(--mj-acc);border-radius:var(--mj-r-xs);color:#fff;font-family:inherit;font-size:var(--mj-fs-body);padding:3px 7px;outline:none}`;
		...
		function Studio() {
			 ...
			 * 故事板的行数据 —— **必须记忆化**。
			 *
			 * 不记忆化的话，每 3 秒的系统状态轮询（loadSys → setSys）都会重渲染
			 * 整棵树，而 shotData() 每次都新建 rows 对象 → 所有镜头卡与其中的
			 * <video> 全部跟着重渲染重绘，观感就是「一闪一闪」。
			 * 故事板不该因为 CPU 数字变了就重画。
			 *
			 * 依赖只列真正影响行的东西；sys 不在这里。
			...
			const shotRows = React.useMemo(
				() => shotData(),
				[proj, prod, job, qcRep, jobRunning],
			);
			...
			function colProjects() {
				...
				return h("div", { className: "mj-col mj-c1", key: "c1" },
					h(Pane, { title: "成品列表", grow: true,
						right: h("span", { className: "mj-tag" }, ps.length + " 部") },
									...
									return h("div", {
										...
										p.cover
											? h("img", {
												...
												alt: "", loading: "lazy", title: "点击放大看封面",
												onClick: (e) => {
													e.stopPropagation();
										...
										h("div", { className: "mj-pinfo" },
											...
											h("div", { className: "mj-pmeta", title: p.style || p.genre || "" },
												shortLabel(p.genre || p.style) + " · " + doneN + "/" + total + " 镜"),
											h("div", { className: "mj-pbar" + (pct >= 100 && total ? " done" : "") },
												h("i", { style: { width: pct + "%" } }))),
										p.hasFinal
											? h("span", { className: "mj-pfinal", title: "已成片" }, "\u2713")
											: null,
										h("button", {
...
function colPipeline() {
			...
			function shotBoard() {
				...
				return h(Pane, { title: "分镜故事板", grow: true,
					...
					shown.length
						? h("div", { className: "mj-boardgrid" }, shown.map((r) => h(ShotCard, {
							key: r.id, row: r, pid: pid, jobRunning: jobRunning,
							// 传**稳定**的回调：内联箭头每次渲染都是新引用，
							// 会让下面的 React.memo 完全失效（等于没 memo）
			...
			function runBar() {
				...
				const shown = logOpen ? lines.slice(Math.max(0, lines.length - 500)) : [];
				...
				return h("div", { className: "mj-run" },
					...
					logOpen
						? h("div", { className: "mj-log", ref: logRef, style: { minHeight: 84, maxHeight: "12vh" } },
							shown.length
								? shown.map((L, i) => h("div", { className: logClass(L), key: i }, L))
			...
			function colProducts() {
				const TABS = [["characters", "人物"], ["scenes", "场景"], ["props", "道具"], ["clips", "镜头"]];
						...
						h(Pane, { title: "ComfyUI 启动管理",
							...
							h("div", { className: "mj-row" },
								...
								h("input", {
									...
									title: "空闲多少分钟后自动退出 ComfyUI；0 = 一直开着（默认）",
									value: P.idleExitMin === undefined ? 0 : P.idleExitMin,
									onChange: (e) => setParam("idleExitMin", e.target.value === "" ? 0 : Number(e.target.value)),
								}),
								h("span", { className: "mj-mut", style: { fontSize: 12 } }, "分钟（0=不退出）")),
							h("div", { className: "mj-mut", style: { fontSize: 12.5 } },
			...
			return h("div", { className: "mj-root" },
				...
				// 风格割裂，也无法把"移入回收站可恢复"讲清楚。
				confirm ? h(Modal, {
					title: "移入回收站", onClose: () => setConfirm(null),
					actions: h("div", { className: "mj-row", style: { gap: 5 } },
						h("button", {
							className: "mj-btn", onClick: () => setConfirm(null),
						}, "取消"),
						h("button", {
							className: "mj-btn", style: { borderColor: "color-mix(in srgb,var(--mj-bad) 55%,transparent)", color: "var(--mj-bad)" },
							onClick: () => doRemoveConfirmed(confirm.id),
						}, "移入回收站")),
				},
					h("div", { className: "mj-mut", style: { fontSize: 13 } },
						"把「" + confirm.title + "」整体移入 ",
						h("span", { className: "mj-mono" }, "_trash"),
						"？"),
					h("div", { className: "mj-mut", style: { fontSize: 12.5 } },
						"项目目录（含已渲染的镜头与成片）会一起移走。**不是永久删除** —— "
						+ "需要时可以从 _trash 里手工恢复。")) : null,
