...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 小说工作区的媒体流（作品封面）。与项目产物分开一条路由，见宿主侧注释。 */
		const NOVELFILE = "/manju-novel-file";
...
.mj-pb,.mj-log,.mj-modalbody,.mj-col{scrollbar-gutter:stable}
/* 列表**不要** scrollbar-gutter:stable —— 那会给右侧永久留一条 12px 空槽（没滚动条时也是空的），
   在成品列表这种窄栏里就是"后面留空这么多"的直接来源。滚动条样式本来就细，直接让它覆盖即可。 */
/* ── 故事板表头 ────────────────────────────────────────────────
...
.mj-chips{display:flex;flex-wrap:wrap;align-items:center;gap:2px 12px}
/* 阶段（节点）标签：原来是 6 个「带底色 + 描边」的圆胶囊，一排下来又花又挤（用户："每个节点标签太丑了"）。
   改成「状态点 + 文字」的极简形态：只有点带颜色，文字常驻中性色，**当前这一步才加重**。 */
.mj-chip{display:flex;align-items:center;gap:6px;padding:0;border:0;background:none;border-radius:0;
  font-size:12px;color:var(--mj-dim);cursor:help;transition:color var(--mj-t)}
.mj-chip .d{width:5px;height:5px;border-radius:50%;background:color-mix(in srgb,var(--mj-dim) 55%,transparent);flex:0 0 auto}
.mj-chip.running{color:color-mix(in srgb,var(--mj-acc) 76%,var(--mj-text));font-weight:600}
.mj-chip.running .d{background:var(--mj-acc);animation:mjPulse 1.2s ease-in-out infinite}
.mj-chip.done{color:color-mix(in srgb,var(--mj-ok) 58%,var(--mj-text))}
.mj-chip.done .d{background:var(--mj-ok)}
.mj-chip.failed{color:color-mix(in srgb,var(--mj-bad) 78%,var(--mj-text));font-weight:600}
.mj-chip.failed .d{background:var(--mj-bad)}
.mj-chip.manual{color:color-mix(in srgb,var(--mj-warn) 76%,var(--mj-text))}
...
.mj-ltag{flex:0 0 auto;font-size:10.5px;line-height:1.5;padding:0 4px;border-radius:3px;
  background:none;color:var(--mj-dim);font-weight:600;letter-spacing:.02em}
...
.mj-grid4>.mj-empty,/* 管线面板按阶段用 3 列；缺这条规则时 .mj-btn.wide{width:100%} 会退化成 6 个整宽按钮竖排，
   把「快速执行/一条龙」挤出侧栏首屏（审计发现）。 */
.mj-grid3{display:grid;grid-template-columns:repeat(3,1fr);gap:7px}
.mj-grid2>.mj-empty{grid-column:1/-1}
...
.mj-dots{flex:0 0 auto;position:static;z-index:3;width:24px;height:24px;padding:0;display:flex;align-items:center;justify-content:center;line-height:1;border:1px solid rgba(255,255,255,.22);border-radius:var(--mj-r-xs);background:rgba(0,0,0,.45);color:rgba(255,255,255,.82);font-family:inherit;font-size:14px;cursor:pointer;
  /* **常驻可见（半透明）**，不再 opacity:0：
     悬停才出现的按钮会让行右侧永远看着是空的（用户："成品列表后面留空这么多干嘛"）。
     用静态提示代替隐藏，同时把命中区用伪元素撑到 40×40（技能库：密集桌面界面 ≥40px 命中区）。 */
  opacity:.5;position:relative;transition:opacity var(--mj-t),background var(--mj-t)}
.mj-dots::after{content:"";position:absolute;inset:-8px}
.mj-dots:hover,.mj-dots.on{opacity:1}
...
.mj-prow{position:relative;display:flex;align-items:center;gap:9px;padding:7px 8px;border:1px solid transparent;
  /* 同心圆角（技能库原则 1）：外层 = 内层 + 内边距 = 6px + 7px ≈ 14px。
     原来外层/内层同为 6px，缩略图在行里会显得"被夹住"。 */
  border-radius:var(--mj-r-lg);cursor:pointer;min-width:0;transition:background var(--mj-t),border-color var(--mj-t)}
...
/* 小说管理：作品封面（点开放大）与卷章清单 */
.mj-ncover{flex:0 0 auto;width:168px;height:96px;border-radius:var(--mj-r-sm);object-fit:cover;background:var(--mj-surface2);outline:1px solid rgba(255,255,255,.10);outline-offset:-1px;cursor:zoom-in}
.mj-chaps{flex:0 0 auto;max-height:34vh;overflow:auto;scrollbar-gutter:stable}
.mj-chapvol{margin-bottom:6px}
.mj-chapvolh{font-size:11.5px;color:var(--mj-dim);margin:2px 0 4px;letter-spacing:.02em}
.mj-pinfo{flex:1 1 auto;min-width:0;display:flex;flex-direction:column;gap:2px}
/* 标题允许两行：单行 + ellipsis 会把「机心问道」截成「机心…」，读不出是哪部剧 */
.mj-ptitle{font-size:13.5px;font-weight:700;line-height:1.35;letter-spacing:.01em;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;text-wrap:balance}
...
/* ⋯ 必须自带定位上下文：基规则里的 ::after{inset:-8px} 以最近的**定位祖先**为包含块，
   一旦本元素是 static，包含块就变成整行 —— 命中区铺满整行，点列表哪儿都弹菜单、项目打不开。
   （审计实测：整行 5 个采样点全部命中 dots。）这里恢复 relative 并把命中区撑到 40×40。 */
.mj-prow .mj-dots{position:relative;width:20px;height:20px;font-size:13px;background:transparent;border-color:transparent;color:var(--mj-dim)}
.mj-prow .mj-dots::after{inset:-10px}
...
.mj-coverrename{position:absolute;left:9px;right:9px;bottom:38px;width:auto;background:rgba(0,0,0,.8);border:1px solid var(--mj-acc);border-radius:var(--mj-r-xs);color:#fff;font-family:inherit;font-size:var(--mj-fs-body);padding:3px 7px;outline:none}`;
		...
		/** 小说作品里的图片/文件（封面）。 */
		function novelUrl(work, rel) {
			return NOVELFILE + "?w=" + encodeURIComponent(work) + "&r=" + encodeURIComponent(rel);
		}

		/** 字数：中文按"万字"读更省地方，不足一万就原样显示。 */
		function fmtWan(n) {
			const v = Number(n) || 0;
			return v >= 10000 ? (Math.round(v / 1000) / 10) + " 万字" : v + " 字";
		}

		/**
		 * 字幕卡类型的显示名。
		 * 与宿主 normCardKind 的白名单**同一套取值**：sys / danmaku / sfx。
		 * 未知值返回空串 = 按普通台词显示（宿主那边也会把未知值丢掉，两边口径一致）。
		 */
		function cardKindLabel(kind) {
			const k = String(kind || "").trim().toLowerCase();
			if (k === "sys" || k === "system") return "系统提示";
			if (k === "danmaku" || k === "dm") return "弹幕";
			if (k === "sfx" || k === "fx") return "音效大字";
			return "";
		}
		...
		function collapseProgress(rows) {
			...
			for (let i = 0; i < rows.length; i++) {
				...
				// 行首的 [标签]（多半是镜头号）**只在它变化时显示**：
				// 每行都顶一个标签就是"一排排一样的小胶囊"，而且镜头块标题里已经写了镜头号。
				const tm = /^\s*\[([^\]]+)\]/.exec(L);
				const tag = tm ? tm[1] : "";
				const showTag = tag !== "" && tag !== prevTag;
				prevTag = tag;
				const m = /^\s*\[([^\]]+)\]\s*渲染中\s+\d+s/.exec(L);
				if (m) {
					if (last && last.tag === m[1]) { last.n++; last.text = L; continue }
					last = { tag: m[1], n: 1, text: L, showTag: showTag };
					out.push(last);
					continue;
				}
				last = null;
				out.push({ tag: tag, n: 0, text: L, showTag: showTag });
			}
			return out;
		}
		...
		function LogLine(props) {
			...
			let rest = L;
			if (mTag) {
				// 标签始终从正文里摘掉；**只有它变化时才画出来**，避免一列重复的小胶囊
				rest = L.slice(mTag[0].length);
				if (props.showTag !== false) {
			...
			for (let i = 0; i < list.length; i++) {
				...
				if (own.length) {
					...
					kids.push(h("div", { className: "mj-lsecbody", key: "own" },
						cols.map((c, k) => h(LogLine, { key: k, text: c.text, n: c.n, showTag: c.showTag }))));
				}
		...
		function Studio() {
			...
			const [liveReverse, setLiveReverse] = React.useState(true);
			...
			// ── 小说管理（同样必须追加在末尾，理由见上）──
			const [nRoot, setNRoot] = React.useState("");
			const [nWorks, setNWorks] = React.useState([]);
			const [nSel, setNSel] = React.useState("");
			const [nDetail, setNDetail] = React.useState(null);
			const [nPre, setNPre] = React.useState(null);
			const [nForm, setNForm] = React.useState(null);
			const [nBusy, setNBusy] = React.useState("");
			...
			// ── 小说管理：作品清单 / 详情 / 正文预览 / 一键做视频 ──

			/**
			 * 拉小说工作区的作品清单。
			 * **懒加载**：只在切到小说管理视图（以及手动刷新）时拉 —— 开机就扫一遍
			 * 几十部作品的目录，会把首屏拖慢，而多数时候用户是来看片子的。
			 */
			const loadNovel = async () => {
				setNBusy("读小说工作区");
				const r = await api("novel.works", {});
				setNBusy("");
				setNRoot((r && r.root) || "");
				setNWorks((r && r.works) || []);
				if (r && r.error) setErr(r.error);
			...
			/** 打开一部作品（详情：立项书 + 逐章清单）。 */
			const openNovelWork = async (work) => {
				setNSel(work);
				setNPre(null);
				const r = await api("novel.work", { work: work });
				if (r && r.error) { setErr(r.error); return; }
				setErr("");
				setNDetail(r);
			};

			/** 预览一个文本文件（章 / 设定集 / 全本）。有界读取，避免把 15 万字塞进 DOM。 */
			const openNovelFile = async (work, rel) => {
				setNBusy("读正文");
				const r = await api("novel.read", { work: work, rel: rel, maxChars: 12000 });
				setNBusy("");
				if (r && r.error) { setErr(r.error); return; }
				setErr("");
				setNPre({ rel: r.rel, text: r.text || "", chars: r.chars, truncated: r.truncated });
			};

			/** 换小说工作区目录（落盘，重启后仍在）。 */
			const setNovelRootDir = async (p) => {
				setNBusy("切换小说目录");
				const r = await api("novel.root", { set: p });
				if (r && r.error) { setNBusy(""); setErr(r.error); return; }
				setErr("");
				const w = await loadNovel();
				setNotice("小说工作区：" + ((w && w.root) || p) + "（" + ((w && w.count) || 0) + " 部作品）");
			};

			/**
			 * 打开「一键做视频」表单。
			 * 默认值就是"最省事且最不容易出错"的那一组：第一章 / 一集 / 开跑 AI 一条龙。
			 */
			const openNovelForm = (w) => {
				const vols = (w && w.vols) || [];
				setNForm({
					work: w.work,
					title: w.title,
			...
			/** 一键：建项目 + 导入本集素材 + 开跑。 */
			const doNovelToVideo = async () => {
				const f = nForm || {};
				setNBusy(f.autoStart === false ? "建项目" : "一键做视频（建项目 + 开跑）");
				const r = await api("novel.toVideo", {
					work: f.work,
					id: f.id,
					title: f.title,
					episode: f.episode,
					...
					genre: f.genre,
					style: f.style,
					scope: { kind: f.kind, vol: f.vol, chapter: f.chapter, count: f.count },
					autoStart: f.autoStart !== false,
					mode: f.mode || "ai",
				});
				setNBusy("");
				if (r && r.error && !r.ok) { setErr(r.error); return; }
				setErr("");
				setNForm(null);
				await loadBoot();
				if (r && r.id) {
					await openProject(r.id);
					setView("video");     // 一键之后把人送到"能看见进度"的地方
				}
				if (r && r.truncated) {
					setNotice("本集用了 " + r.chars + " 字正文（原作共 " + r.totalChars
						+ " 字）。分镜那一步一次只读前 " + r.maxChars + " 字，剩下的内容留给下一集，不会丢。");
				} else if (r && r.warning) {
					setNotice(r.warning);
				} else if (r && r.id) {
					setNotice("已建项目 " + r.id + " · " + r.episode + "「" + r.episodeName + "」"
						+ (r.jobId ? "，已在后台开跑（进度看下面的运行条）" : "（未开跑）"));
				}
			};
			 ...
			 * 停止"不是本页面发起"的宿主作业。
			 *
			 * 为什么需要：从「一键做视频」起的管线是**宿主动手**起的，本页面手里没有 jobId；
			 * 刷新过页面之后也一样。原来这种作业既看不见、也停不掉，只能重启 DSH（实测踩过）。
			 */
			const stopHostJob = async (jobId) => {
				const r = await api("pipelineStop", { jobId: jobId });
				if (r && r.error) { setErr(r.error); return; }
				setErr("");
				setNotice("已停止：" + (((r && r.stopped) || []).join("、") || jobId));
				const a = await api("activity", {});
				if (a && !a.error) setAct(a);
			};
			...
			// 切到「小说管理」时才去扫小说工作区（懒加载）。
			// 开机就扫几十部作品的目录会把首屏拖慢，而多数时候用户是来看片子的。
			React.useEffect(() => {
				if (view === "novel" && !nWorks.length && !nBusy) loadNovel();
			}, [view]);
			...
			React.useEffect(() => {
				...
				if (!el) return;
				// **只在用户本来就贴着边时才跟随**：原来每落一行都强制归位，
				// 用户想回看上一阶段必须先停下作业（审计指出：这等于"永远只看得到最后一行"）。
				const atTop = el.scrollTop <= 24;
				const atBottom = el.scrollHeight - el.scrollTop - el.clientHeight <= 24;
				if (liveReverse ? atTop : atBottom) el.scrollTop = liveReverse ? 0 : el.scrollHeight;
...
function colPipeline() {
					...
					dialogue.length
						? h("div", null,
							...
							dialogue.map((d, i) => h("div", { className: "mj-mono", key: i },
								// 字幕卡要能一眼认出来（哪句在左上、哪句是弹幕、哪句是音效大字），
								// 否则作者只能靠"这行字看起来像系统提示"去猜。
								(cardKindLabel(d.kind) ? h("span", { className: "mj-ltag" }, cardKindLabel(d.kind)) : null),
				...
				return h("div", { className: "mj-run" },
					h("div", { className: "mj-runbar" },
						...
						total ? h("span", { className: "mj-mut", style: { fontSize: 12.5, fontVariantNumeric: "tabular-nums" } },
							idx + "/" + total + " · " + pct + "%") : null,
						// 不是本页面发起的作业（一键做视频起的 / 刷新后认领的）也要能停 ——
						// 卡住的作业现在点了就立即放行，不必再重启 DSH。
						ext && act && act.hostJobId
							? h("button", {
								className: "mj-btn", disabled: !act.running,
								title: "停止这个作业（卡住的作业会立即放行）",
								onClick: () => stopHostJob(act.hostJobId),
							}, "\u25a0 停止")
							: null,
						h("span", { className: "mj-sp" }),
						h("button", {
			...
			// ── 视图：小说管理 ──
			//
			// 版式：作品列表（窄）｜ 作品详情 + 卷章 + 正文预览（主区，可滚动）。
			// 与「视频管理」的分工：那边是"要交付的片子"（项目），这边是"还没变成片子的原料"
			// （小说工作区里的作品）；两者之间的桥，就是每部作品右上角那个「一键做视频」。
			function viewNovel() {
				const d = nDetail;
				const vols = (d && d.volList) || [];
				return h("div", { className: "mj-views mj-novel" },
					h("div", { className: "mj-col mj-c1", key: "nl" },
						h(Pane, { title: "小说作品", grow: true,
							right: h("div", { className: "mj-row", style: { gap: 4, flex: "0 0 auto" } },
								h("span", { className: "mj-tag", title: nRoot || "" }, nWorks.length + " 部"),
								h("button", {
									className: "mj-refresh ic" + (nBusy ? " busy" : ""),
									disabled: !!nBusy,
									title: "重新扫描小说工作区",
									onClick: loadNovel,
								}, h("span", { className: "mj-refi" }, "\u21bb"))) },
							h("div", { className: "mj-plist" },
								nWorks.length
									? nWorks.map((x) => h("div", {
										key: x.work,
										className: "mj-prow" + (x.work === nSel ? " on" : ""),
										title: x.title + "（" + x.work + "）",
										onClick: () => openNovelWork(x.work),
									},
										x.cover
											? h("img", {
												className: "mj-pthumb", src: novelUrl(x.work, x.cover),
												alt: "", loading: "lazy", decoding: "async",
											})
											: h("div", { className: "mj-pthumb mj-pthumb-ph" }, "\u2725"),
										h("div", { className: "mj-pinfo" },
											h("div", { className: "mj-ptitle" }, x.title),
											h("div", { className: "mj-pmeta" },
												[
													Number(x.chapterCount) ? x.chapterCount + " 章" : "没有正文",
													fmtWan(x.chars),
													Number(x.volCount) ? x.volCount + " 卷" : "",
													x.full ? "有全本" : "",
												].filter(Boolean).join(" · ")),
											h("div", { className: "mj-mut", style: { fontSize: 11.5 } },
												shortLabel(x.genre || "") || (x.hasMeta ? "—" : "无立项书")))))
									: h("div", { className: "mj-empty" },
										h("span", null, "\u2725"),
										"小说工作区里还没有作品",
										h("div", { className: "mj-mut", style: { maxWidth: 320 } },
											"工作区：" + (nRoot || "—")))))),
					h("div", { className: "mj-col mj-grow", key: "nr" },
						h(Pane, { title: "小说工作区",
							right: h("span", { className: "mj-tag" }, nBusy || (nRoot || "—")) },
							h("div", { className: "mj-row", style: { flexWrap: "nowrap" } },
								h("input", {
									className: "mj-in", value: nRoot,
									placeholder: "小说工作区绝对路径，如 D:\\Ai\\小说",
									onChange: (e) => setNRoot(e.target.value),
								}),
								h("button", {
									className: "mj-btn", disabled: !!nBusy || !nRoot,
									onClick: () => setNovelRootDir(nRoot),
								}, "切换")),
							h("div", { className: "mj-mut", style: { fontSize: 12.5 } },
								"作品 = 工作区下的一个目录：立项.json + 正文/<卷>/第NNN章_*.md + 设定集 + 全本 + 封面。")),

						d
							? h(Pane, { title: "作品：" + (d.title || d.work),
								right: h("div", { className: "mj-row", style: { gap: 6, flex: "0 0 auto" } },
									h("span", { className: "mj-tag" },
										(d.chapterCount || 0) + " 章 · " + fmtWan(d.chars)),
									h("button", {
										className: "mj-btn pri", disabled: !!nBusy || !d.chapterCount,
										title: d.chapterCount ? "选好范围，建项目并开跑" : "这部作品还没有正文",
										onClick: () => openNovelForm(d),
									}, "\u26a1 一键做视频")) },
								h("div", { className: "mj-row", style: { alignItems: "flex-start", gap: 12 } },
									d.cover
										? h("img", {
											className: "mj-ncover mj-zoomable", tabIndex: 0, role: "button",
											src: novelUrl(d.work, d.cover), alt: "", title: "点击放大看封面",
											onKeyDown: (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); if (e.currentTarget.click) e.currentTarget.click(); } },
											onClick: () => openViewer([{ src: novelUrl(d.work, d.cover), label: (d.title || d.work) + " 封面" }], 0),
										})
										: null,
									h("div", { className: "mj-grow", style: { minWidth: 0 } },
										h("div", { className: "mj-mono mj-mut", style: { fontSize: 12.5 } },
											[d.genre, d.route, d.depth].filter(Boolean).join(" · ")
											+ (d.createdAt ? " · 立项 " + d.createdAt : "")),
										h("div", { style: { fontSize: 13, marginTop: 6, lineHeight: 1.5 } },
											d.hook || d.logline || "（立项书里没有高概念）"),
										h("div", { className: "mj-mono mj-mut", style: { fontSize: 12, marginTop: 6 } },
											(d.protagonist ? "主角 " + d.protagonist + " · " : "")
											+ (d.full ? "全本 " + fmtWan(d.full.chars) : "还没有全本")
											+ (d.settings.length ? " · 设定集 " + d.settings.length + " 份" : "")),
										d.art
											? h("div", { className: "mj-mut", style: { fontSize: 12 } }, "美术方向：" + d.art)
											: null)))
							: h(Pane, { title: "作品", grow: true },
								h("div", { className: "mj-empty" },
									h("span", null, "\u2261"),
									"从左边选一部作品",
									h("div", { className: "mj-mut", style: { maxWidth: 380 } },
										"选好之后这里会列出分卷与逐章正文，右上角的「⚡ 一键做视频」会按你选的范围建项目并开跑。"))),

						d && vols.length
							? h(Pane, { title: "内容", grow: true, style: { minHeight: 0 },
								right: d.full
									? h("button", {
										className: "mj-btn", title: d.full.rel,
										onClick: () => openNovelFile(d.work, d.full.rel),
									}, "读全本")
									: null },
								h("div", { className: "mj-chaps" },
									vols.map((v, vi) => h("div", { className: "mj-chapvol", key: v.vol || ("v" + vi) },
										h("div", { className: "mj-chapvolh" },
											(v.vol || "正文") + " · " + v.chapters.length + " 章 · " + fmtWan(v.chars)),
										h("div", { className: "mj-row", style: { flexWrap: "wrap", gap: 4 } },
											v.chapters.map((c) => h("button", {
												key: c.rel,
												className: "mj-btn" + (nPre && nPre.rel === c.rel ? " pri" : ""),
												title: c.rel + " · " + c.chars + " 字",
												onClick: () => openNovelFile(d.work, c.rel),
											}, c.title)))))),
								d.settings.length
									? h("div", { className: "mj-row", style: { flexWrap: "wrap", gap: 4, marginTop: 8 } },
										h("span", { className: "mj-lab", style: { flex: "0 0 auto" } }, "设定集"),
										d.settings.map((s) => h("button", {
											key: s.name,
											className: "mj-btn" + (nPre && nPre.rel === "设定集/" + s.name ? " pri" : ""),
											onClick: () => openNovelFile(d.work, "设定集/" + s.name),
										}, s.name.replace(/\.(md|txt)$/i, ""))))
									: null,
								h("div", { className: "mj-pre mj-grow", style: { minHeight: 160, marginTop: 8 } },
									nPre
										? h("div", null,
											h("div", { className: "mj-mut", style: { fontSize: 11.5, marginBottom: 4 } },
												nPre.rel + " · " + nPre.chars + " 字"
												+ (nPre.truncated ? "（只预览前 1.2 万字）" : "")),
											nPre.text)
										: h("div", { className: "mj-mut" }, "点上面的章 / 设定集 / 全本，正文显示在这里")))
							: null));
			...
			const nChapList = [];
			{
				const vs = (nDetail && nDetail.volList) || [];
				for (let i = 0; i < vs.length; i++) {
					for (let k = 0; k < vs[i].chapters.length; k++) {
						nChapList.push(Object.assign({ vol: vs[i].vol }, vs[i].chapters[k]));
					}
				}
			}

			// ── 外壳 ──
			// 三视图对齐 NiliX（视频管理 / 小说管理 / ComfyUI）。
			// 「小说管理」是原来只写在注释里、一直没做的那个 —— 小说侧的作品与
			// "一键做视频"得有自己的家，而不是塞在"新建项目"弹窗的某个输入框里。
			const NAV = [["video", "视频管理"], ["novel", "小说管理"], ["comfy", "ComfyUI"]];
			...
			return h("div", { className: "mj-root" },
				...
				h("div", { className: "mj-head" },
					h("h1", { className: "mj-h1" },
						view === "comfy" ? "ComfyUI" : (view === "novel" ? "小说管理" : "视频管理")),
					h("span", { className: "mj-sub" },
						"漫剧工作台 · 一站式管线 · "
						+ (view === "video" ? "成品列表" : (view === "novel" ? "小说 → 漫剧" : "本地 H3 渲染"))),
					h("span", { className: "mj-sp" }),
				...
				// 兜底分发：不是 comfy / novel 就当视频管理。写成穷举三元的话，
				// view 取到任何非预期值（旧会话残留等）都会整片空白。
				view === "comfy" ? viewComfy() : (view === "novel" ? viewNovel() : viewVideo()),
				...
				nForm ? h(Modal, { title: "一键做视频", onClose: () => setNForm(null) },
					h("div", { className: "mj-mut", style: { fontSize: 13 } },
						"把「" + (nForm.title || nForm.work) + "」做成漫剧。点下面的按钮后，会自动做完这四步："),
					h("div", { className: "mj-mono mj-mut", style: { fontSize: 12.5, lineHeight: 1.7 } },
						"① 建项目（一部小说 = 一个项目；第二次做第二集会落进同一个项目，集号自动往后排）\n"
						+ "② 按上面选的「素材范围」取正文，存成本集素材（不动其它集的内容）\n"
						+ "③ 登记集号与集名（片头卡上显示的那个）\n"
						+ "④ 开跑 AI 一条龙：分镜方案 → 定妆资产 → 渲染 → 质检 → 合成（要占显卡，可关掉）"),
					h(KV, { label: "项目 id" },
						h("input", {
							className: "mj-in", value: nForm.id,
							placeholder: "留空 = 自动取一个（如 manju-1gqm2q）并记住这部作品对应它",
							onChange: (e) => setNForm(Object.assign({}, nForm, { id: e.target.value })),
						})),
					h("div", { className: "mj-mut", style: { fontSize: 12 } },
						"项目 id 只能用字母/数字/下划线/连字符，所以中文书名会自动换成一串短码；"
						+ "想要好认的名字（如 xitai-tiantai）就在这里手填，之后做下一集不用再填。"),
					h(KV, { label: "标题" },
						h("input", {
							className: "mj-in", value: nForm.title,
							onChange: (e) => setNForm(Object.assign({}, nForm, { title: e.target.value })),
						})),
					h(KV, { label: "素材范围" },
						h("div", { className: "mj-row", style: { flexWrap: "wrap", gap: 4 } },
							[["chapter", "按章"], ["volume", "整卷"], ["full", "全本"]].map((k) => h("button", {
								key: k[0],
								className: "mj-btn" + (nForm.kind === k[0] ? " pri" : ""),
								onClick: () => setNForm(Object.assign({}, nForm, { kind: k[0] })),
							}, k[1])))),
					nForm.kind === "volume"
						? h(KV, { label: "选卷" },
							h("select", {
								className: "mj-in", value: nForm.vol,
								onChange: (e) => setNForm(Object.assign({}, nForm, { vol: e.target.value })),
							},
								((nDetail && nDetail.volList) || []).map((v) => h("option", { key: v.vol, value: v.vol },
									(v.vol || "正文") + "（" + v.chapters.length + " 章 · " + fmtWan(v.chars) + "）"))))
						: (nForm.kind === "chapter"
							? h(KV, { label: "起始章" },
								h("select", {
									className: "mj-in", value: nForm.chapter,
									onChange: (e) => setNForm(Object.assign({}, nForm, { chapter: e.target.value })),
								},
									h("option", { value: "" }, "从第一章开始"),
									nChapList.map((c) => h("option", { key: c.rel, value: c.rel },
										c.title + "（" + c.chars + " 字）"))),
								h("span", { className: "mj-lab", style: { flex: "0 0 auto" } }, "连续"),
								h("input", {
									className: "mj-in", style: { width: 56 }, type: "number", min: 1, max: 30,
									value: nForm.count,
									onChange: (e) => setNForm(Object.assign({}, nForm, { count: e.target.value })),
								}),
								h("span", { className: "mj-mut" }, "章"))
							: h("div", { className: "mj-mut", style: { fontSize: 12.5 } },
								"全本优先读 全本/ 里的合并稿。注意：分镜那一步（管线里的「方案」）一次只会读正文的前 1.2 万字，"
								+ "所以一集大约只够 1–2 章 —— 多出来的内容不是丢了，是留给下一集。"
								+ "建完之后会明确告诉你「这次实际用了多少字」。")),
					h(KV, { label: "集号" },
						h("input", {
							className: "mj-in", style: { width: 96 }, value: nForm.episode,
							placeholder: "留空 = 自动排下一集",
							onChange: (e) => setNForm(Object.assign({}, nForm, { episode: e.target.value })),
						}),
						h("span", { className: "mj-lab", style: { flex: "0 0 auto" } }, "集名"),
						h("input", {
							className: "mj-in", value: nForm.epName,
							placeholder: "留空 = 用范围标题（如 第001章 八文一斤）",
							onChange: (e) => setNForm(Object.assign({}, nForm, { epName: e.target.value })),
						})),
					h(KV, { label: "风格句" },
						h("input", {
							className: "mj-in", value: nForm.style,
							placeholder: "留空 = 用默认的「3D 动漫脸 + 干净画面」风格句（之后可在「参数」里改）",
							onChange: (e) => setNForm(Object.assign({}, nForm, { style: e.target.value })),
						})),
					h("div", { className: "mj-row" },
						h("button", {
							className: "mj-btn" + (nForm.autoStart ? " go" : ""),
							title: "建完项目就直接跑 AI 一条龙（方案 → 资产 → 渲染 → 质检 → 合成）",
							onClick: () => setNForm(Object.assign({}, nForm, { autoStart: !nForm.autoStart })),
						}, nForm.autoStart ? "建完就开跑：开" : "建完就开跑：关"),
						h("span", { className: "mj-mut", style: { fontSize: 12.5 } },
							nForm.autoStart ? "会立刻占用显卡渲染（一集通常几十分钟到几小时）" : "只建项目与素材，之后在「视频管理」里手动开跑")),
					h("div", { className: "mj-row" },
						h("button", { className: "mj-btn pri", disabled: !!nBusy, onClick: doNovelToVideo },
							nForm.autoStart ? "\u26a1 建项目并开跑" : "\u26a1 只建项目"),
						h("button", { className: "mj-btn", onClick: () => setNForm(null) }, "取消"))) : null,
