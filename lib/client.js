...
/* ── 全局作业监视条 ──
   浮在底部、不随项目切换而消失；最新一行在最上面，所以不滚动也总能看到进度。 */
.mj-act{position:fixed;left:50%;transform:translateX(-50%);bottom:14px;width:min(1000px,94vw);z-index:60;
  border:1px solid var(--mj-line);border-radius:var(--mj-r-md);background:rgba(9,11,15,.96);
  box-shadow:0 14px 38px rgba(0,0,0,.55);overflow:hidden;backdrop-filter:blur(6px)}
.mj-act.on{border-color:rgba(80,220,140,.45)}
.mj-actbar{display:flex;align-items:center;gap:8px;padding:7px 10px;border-bottom:1px solid var(--mj-line)}
.mj-actdot{flex:0 0 auto;width:8px;height:8px;border-radius:50%;background:var(--mj-dim)}
.mj-actdot.on{background:var(--mj-ok);box-shadow:0 0 0 3px rgba(80,220,140,.16);animation:mj-pulse 1.6s ease-in-out infinite}
@keyframes mj-pulse{0%,100%{opacity:1}50%{opacity:.3}}
.mj-actlog{margin:0;padding:8px 10px;max-height:30vh;overflow:auto;white-space:pre-wrap;word-break:break-all;
  font-family:ui-monospace,SFMono-Regular,Consolas,monospace;font-size:11.5px;line-height:1.5;color:var(--mj-text)}
...
function colPipeline() {
			...
			return h("div", { className: "mj-root" },
				...
				// 全局作业监视条：只要有任何管线在跑就浮在底部（**不看当前选的是哪个项目**）。
				// 最新一行在最上面，所以不用滚动也总能看到进度（渲染每 30 秒会打一行）。
				(act && act.active && (act.running || act.stale)) ? h("div", {
					className: "mj-act" + (act.running ? " on" : ""),
				},
					h("div", { className: "mj-actbar" },
						h("span", { className: "mj-actdot" + (act.running ? " on" : "") }),
						h("b", { style: { fontSize: 12.5 } }, act.running ? "正在跑" : "已中断"),
						h("span", { className: "mj-mut", style: { fontSize: 11.5 } },
							[act.active.project, act.active.cmd, act.active.iso].filter(Boolean).join(" · ")),
						h("span", { className: "mj-sp" }),
						act.running
							? h("span", { className: "mj-mut", style: { fontSize: 11.5 } },
								"已跑 " + fmtDur(Math.max(0, (Date.now() - (act.active.startedAt * 1000 || Date.now())) / 1000)))
							: h("span", { className: "mj-mut", style: { fontSize: 11.5 } },
								"进程已不在（例如桌面端重启把它带走了）"),
						h("button", {
							className: "mj-btn", style: { padding: "2px 8px" },
					...
					actOpen ? h("pre", { className: "mj-actlog" },
						(act.tail || "（还没有输出）").split("\n").filter((x) => x !== "").slice(-60).reverse().join("\n"))
						: null)
					: null,
