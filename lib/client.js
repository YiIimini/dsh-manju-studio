...
.mj-coverrename{position:absolute;left:9px;right:9px;bottom:38px;width:auto;background:rgba(0,0,0,.8);border:1px solid var(--mj-acc);border-radius:var(--mj-r-xs);color:#fff;font-family:inherit;font-size:var(--mj-fs-body);padding:3px 7px;outline:none}`;
		...
		function Studio() {
			...
			const doNovelToVideo = async () => {
				...
				if (r && r.truncated) {
					setNotice("本集进入分镜 " + r.chars + " 字（原作共 " + r.totalChars
						+ " 字，方案阶段上限 " + r.maxChars + " 字，超出的内容留给下一集）");
				} else if (r && r.warning) {
					setNotice(r.warning);
				} else if (r && r.id) {
...
function colPipeline() {
			...
			return h("div", { className: "mj-root" },
				...
				nForm ? h(Modal, { title: "一键做视频", onClose: () => setNForm(null) },
					h("div", { className: "mj-mut", style: { fontSize: 13 } },
						"把「" + (nForm.title || nForm.work) + "」做成漫剧。"
						+ "一部小说 = 一个项目，每一集取一段正文 —— 第二次做第二集会自动落进同一个项目，"
						+ "集号也会自动往后排。"),
					h(KV, { label: "项目 id" },
						h("input", {
							className: "mj-in", value: nForm.id,
							placeholder: "留空 = 自动生成（并记住这部作品对应哪个项目）",
					...
					nForm.kind === "volume"
						...
						: (nForm.kind === "chapter"
							...
							: h("div", { className: "mj-mut", style: { fontSize: 12.5 } },
								"全本优先读 全本/ 里的合并稿。注意：方案阶段只会把正文的前 1.2 万字交给模型，"
								+ "多出来的部分要下一集才用得上 —— 建完之后会明确告诉你实际进了多少字。")),
					h(KV, { label: "集号" },
						h("input", {
					...
					h(KV, { label: "风格句" },
						h("input", {
							...
							placeholder: "留空 = 用默认的 3D 动漫风格句（服务端持有唯一真源）",
