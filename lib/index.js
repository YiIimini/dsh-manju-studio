...
function applyInner(ctx) {
  ...
  function coverBrief(meta, params, plan) {
    ...
    // 取作品自己的场景当取景对象（最多两个，避免拼贴感）：封面才和正片是一套世界观
    const scenes = []
    const sc = (plan && plan.scenes) || []
    for (let i = 0; i < sc.length && scenes.length < 2; i++) {
      const d = String((sc[i] && sc[i].description) || '').trim()
      if (d) scenes.push(d)
    }
    if (scenes.length) parts.push('SETTING — drawn from this series\' own scene design: ' + scenes.join(' / '))
    if (custom) parts.push('Additional art direction: ' + custom)
    parts.push('COMPOSITION: environment-first wide shot that captures the mood of the whole episode — a strong sense of '
      + 'place, dramatic light and atmosphere, deep depth staging, clear silhouette and colour separation, rich '
      + 'incidental detail. A small distant figure may appear only for scale, seen from behind or as an unlit silhouette, '
      + 'never showing a readable face. Leave the upper third calm and uncluttered so an episode title can be overlaid later.')
    parts.push('MUST NOT: no character portrait, no close-up face, no front-facing bust, no cast line-up, no beauty '
      + 'close-up, no collage, no split screen, no market street, no shop fronts, no hanging signboards, no plaques, '
      + 'no banners, no scrolls, no text, no lettering, no calligraphy, no glyphs, no symbols that look like writing, '
      + 'no logo, no watermark, no border, no UI.'
