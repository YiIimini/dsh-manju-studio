...
/** 小说工作区的封面/素材流。与项目产物分开一条路由，见该路由处注释。 */
const NOVEL_FILE_PATH = '/manju-novel-file'
/** 小说工作区默认根目录（用户 2026-09-23 指定的目录）。 */
const NOVEL_ROOT_DEFAULT = 'D:\\Ai\\小说'
/** 作品目录名：中文/空格都允许，但不许出现路径分隔符或 ..（那是穿越的入口）。 */
const WORK_RE = /^[^\\/:*?"<>|]{1,80}$/
/** 小说工作区配置落盘位置（项目根下的隐藏配置文件，不参与作品列表）。 */
const STUDIO_CFG = '_studio.json'
/**
 * 方案阶段的正文上限。
 *
 * genPlan 里 `src.text.slice(0, 12000)` 是硬事实：喂进去 15 万字也只有前 12000 字被看见。
 * 所以"一键做视频"选素材范围时按这个数截断，并把截断这件事**明确报给用户** ——
 * 而不是假装整本书都进了分镜。
 */
const PLAN_INPUT_MAX = 12000
/**
 * 方案阶段（大模型）的两道时间闸。
 *
 * 事故（2026-09-23，用户遇到）：`llm.stream()` 会在**既没有输出、也不报错**的情况下永久挂着
 * （原实现就是 `for await (const c of stream)`，没有任何超时）。于是作业的 running 永远是 true，
 * 界面从此只会说「已有一条管线在运行」，却看不到任何日志（宿主原先只在作业**结束**时才落盘日志），
 * 用户被永久挡住，除了重启 DSH 没有出路。
 * 空闲闸治"卡住不动"，总闸治"一直在慢慢吐、但永远吐不完"。
 */
const LLM_IDLE_MS = 120000
const LLM_TOTAL_MS = 900000
/**
 * 作业看门狗：这么久没有任何新输出，就判定卡死并强制收尾。
 * 渲染时每 30 秒就有进度行，资产/合成阶段也都有输出 —— 十分钟静默必是死了。
 * 可用 MANJU_JOB_STALE_SEC 覆盖（测试要把阈值压到秒级才好验）。
 */
const JOB_STALE_MS = Math.max(1, Number((typeof process !== 'undefined' && process.env && process.env.MANJU_JOB_STALE_SEC) || 0) || 600) * 1000
/** 看门狗的巡检周期（测试会压到毫秒级）。 */
const JOB_TICK_MS = Math.max(200, Number((typeof process !== 'undefined' && process.env && process.env.MANJU_JOB_TICK_MS) || 0) || 20000)
 ...
 * 「一键做视频」的默认风格句。
 *
 * 这是用户 2026-09-23 明确的美术方向（3D 动漫脸、画面干净正常、电影级布光），
 * 也是经过实测的写法：**英文 + 前置**。中文风格描述实测不生效，而把风格句放到提示词
 * 中后段会被主体描述盖掉 —— 所以它必须原样前置到每一镜的最开头。
 * 界面上可改：这是默认值，不是硬编码的死值。
 */
const DEFAULT_STYLE_3D = 'STYLE (mandatory, top priority): STYLIZED 3D ANIMATION IN THE CHINESE DONGHUA MANNER. Characters have clear ANIMATED faces — large expressive eyes with visible highlights, simplified noses and mouths, softly rounded non-realistic proportions, a slightly graphic look to hair and skin shading. The faces must NEVER look like real photographic human beings: no photoreal skin pores, no real-person likeness, no celebrity resemblance, no photographic realism of any kind — this is a stylized animated character, always. Surfaces are clean and normal: tidy painted metal, matte rubber, ordinary fabric, glass and wet pavement rendered cleanly, without grime, rust, oil or decay as a theme. Lighting is cinematic and generous — practical glow, shallow depth of field, gentle bloom, believable volume haze; nothing grubby, nothing spotless-plastic either: it should look like a well-made modern animated series. MUST NOT: photorealistic or live-action-looking faces, real-person likeness, celebrity resemblance, 2D flat cel shading, glossy mobile-game rendering, plastic airbrushed surfaces, heavy grime/rust/oil dirt as a style, decorative borders, paper texture, any frame-in-frame decoration.'

/**
 * 字幕卡类型白名单：`sys` 系统提示 / `danmaku` 弹幕 / `sfx` 音效大字。
 *
 * 为什么必须是白名单而不是"原样透传"：未知 kind 一律退回普通对白。
 * 模型随手编一个 kind 也不该在画面上留下一个位置诡异的字块 ——
 * 字幕卡是"位置上很显眼"的东西，宁可少一张，不能错一张。
 */
function normCardKind(v) {
  const k = String(v === undefined || v === null ? '' : v).trim().toLowerCase()
  if (k === 'sys' || k === 'system') return 'sys'
  if (k === 'danmaku' || k === 'dm') return 'danmaku'
  if (k === 'sfx' || k === 'fx') return 'sfx'
  return ''
}

/**
 * 字幕卡的 ASS 行（sys / danmaku / sfx）。
 *
 * 与驱动器 `build_ass` **逐字同口径**：两半都会出字幕，一处改了另一处没改，
 * 就会出现"界面合成的成片和命令行合成的成片、字幕不一样"这种最难查的差异。
 * 用行内覆盖标签实现，**不动 Style 表** —— 不需要新模板，也不影响已有项目。
 *
 * 注意 sfx 的 Style 名沿用「对白/旁白」而不是自造「音效」：ASS 里没有定义这个 Style 时，
 * libass 会回落到 Default，未覆盖的属性（边距等）就跟另一条通路不一致了。
 */
function assCard(kind, txt, o) {
  const size = Number(o.size) || 38
  const W = Number(o.width) || 1344
  const H = Number(o.height) || 768
  const fade = Number(o.fadeMs) || 0
  if (kind === 'sys') {
    return {
      style: '系统',
      text: '{\\an7\\pos(' + Math.round(W * 0.05) + ',' + Math.round(H * 0.06) + ')\\fs' + Math.round(size * 0.66)
        + '\\c&H60E0D0&\\bord3\\3c&H00000000&\\fad(' + fade + ',' + fade + ')}' + txt,
    }
  }
  if (kind === 'danmaku') {
    return {
      style: '弹幕',
      text: '{\\an8\\pos(' + Math.round(W / 2) + ',' + Math.round(H * 0.04) + ')\\fs' + Math.round(size * 0.52)
        + '\\c&HA8A8A8&\\fad(' + fade + ',' + fade + ')}' + txt,
    }
  }
  if (kind === 'sfx') {
    return {
      style: '音效',
      text: '{\\an5\\pos(' + Math.round(W / 2) + ',' + Math.round(H / 2) + ')\\fs' + Math.round(size * 2.2)
        + '\\c&HFFFFFF&\\bord7\\3c&H00000000&\\fad(60,140)}' + txt,
    }
  }
  return null
}
...
function applyInner(ctx) {
  ...
  /**
   * 相对路径 → 项目根下的绝对路径；**已经是绝对路径的原样放行**。
   *
   * 为什么允许绝对路径：小说工作区（`D:\Ai\小说`）在项目根（`D:\Ai\漫剧`）之外，
   * 而它的作品目录同样要用 readText/listDir/writeText 这套助手。
   * 与其为"另一个根"复制一整套助手（迟早漂移），不如让 abs() 认得绝对路径 ——
   * 判据是盘符开头，拼出来的相对路径永远不可能是这个形状，所以不会误伤。
   */
  const abs = (rel) => {
    if (!rel) return ROOT
    return /^[A-Za-z]:\\/.test(rel) ? rel : ROOT + '\\' + rel
  }
   ...
   * 写盘时的**按调用沙箱策略**。
   *
   * 为什么必须显式给：`ctx.fs` 实际是 `@deepseek-ai/dsh-fs-sandbox`，
   * 它按"每次调用的策略"做包含性校验（`writeText(target, content, expected, signal, sandboxPolicy)`）；
   * **不传策略就套用部署默认**（workspace-write + 会话工作区根），
   * 而本插件管的是 `D:\Ai\漫剧`（项目）与小说工作区（默认 `D:\Ai\小说`）——
   * 都在会话工作区之外。实测后果：界面里点「一键做视频」，第一步就报
   * `cannot write "D:\Ai\漫剧\manju-xxxx\project.json": file access denied under workspace-write mode`。
   *
   * 口径：**只为自己拥有的两个根盖章**（谁的地盘写谁的名），其余路径一律不盖章 ——
   * 那样仍然走部署默认的围栏；未知路径不该被这个插件放行。
   */
  async function writePolicyFor(absPath) {
    const low = String(absPath || '').toLowerCase()
    const roots = [ROOT]
    try { roots.push(await novelRootPath()) } catch (e) { /* 读不到小说工作区配置就不算它的地盘 */ }
    for (let i = 0; i < roots.length; i++) {
      const raw = String(roots[i] || '').replace(/[\\/]+$/, '')
      if (!raw) continue
      const r = raw.toLowerCase()
      if (low === r || low.indexOf(r + '\\') === 0) return { mode: 'workspace-write', workspaceRoot: raw }
    }
    return undefined
  }

  /** 写文件（带沙箱策略戳）。失败若是被沙箱拦下，用**人话**说清"发生了什么 + 怎么办"。 */
  async function writeFileStamped(pAbs, text) {
    const stamp = await writePolicyFor(pAbs)
    try {
      await fs.writeText(await fs.resolve(pAbs), text, undefined, undefined, stamp)
    } catch (e) {
      const msg = String((e && e.message) || e)
      if (/access denied|FS_SANDBOX_DENIED/i.test(msg)) {
        // 界面上的红条不该出现"workspace-write mode"这种词 —— 那是给开发者看的。
        // 用户要的是：出什么事了、我该做什么、以及一条能直接复制给我的原始信息。
        throw new Error('写文件被 DSH 的安全沙箱拦下了。'
          + '漫剧工作台只在自己管的两处目录里写文件（' + ROOT + ' 与小说工作区'
          + (novelRootMemo ? ' ' + novelRootMemo : '') + '），这次要写的位置不在其中，'
          + '通常是插件版本问题。请点这条消息右上角的「复制」，把它发给我。'
          + '（原始信息：' + msg + '）')
      }
      throw e
    }
  ...
  async function writeText(rel, text) {
    ...
    await writeFileStamped(p, text)
  }

  /**
   * 诊断头必须**纯 ASCII**。
   *
   * 事故（2026-09-22 实测复现）：头值里带中文时 `res.writeHead` 抛 ERR_INVALID_CHAR。
   * 而项目根就是 `D:\Ai\漫剧`，所以几乎每条错误诊断都含中文 —— 更糟的是错误分支里
   * 再写一次同样的头又抛一次，被内层 `catch { /* 已响应 *\/ }` 静默吞掉，于是
   * **响应永远发不出去**：浏览器端表现为图片一直转圈、接口一直挂起，而服务端日志干净。
   * 统一转成 \uXXXX 转义，谁都看得懂，也不会再炸。
   */
  function diagH(v) {
  ...
  /**
   * 任务日志落盘 → <项目>/output/logs/<name>-<时间戳>.log。
   *
   * 为什么必须落盘：渲染/管线的日志此前只活在插件进程内存里（job.log），
   * 界面一刷新、进程一重启就没了。而"这一镜为什么渲了 8 分钟""质检报了哪一条"
   * 恰恰是事后才要查的东西。落盘之后 `logs.list` / `logs.read` 能翻旧账。
   */
  async function saveJobLog(project, name, text) {
    try {
      if (!text) return ''
      const stamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19)
      const rel = 'output/logs/' + String(name || 'job') + '-' + stamp + '.log'
      await writeText(project + '\\' + rel.replace(/\//g, '\\'), String(text))
      return rel
    } catch (e) {
      return ''
    }
  ...
  async function ensurePosters(id, clips) {
    ...
    try {
      // 同样要带沙箱策略戳：不带会被按部署默认拦下（本项目根不在会话工作区里）
      await writeFileStamped(abs(id + '\\_posters\\.keep'), '')
  ...
  async function summary(id) {
    ...
    let assetCount = 0
    // 成品列表封面：**独立封面优先**。
    // 没有独立封面时退回场景图 / 道具图 —— **绝不拿角色定妆照当封面**（用户明确要求）：
    // 定妆照是 Ref2VA 的身份锚点，正面半身 + 中性背景，代表的是"某个人"而不是这部片子。
    let cover = await coverOf(id)
    for (let k = 0; k < KINDS.length; k++) {
      const arr = assets[KINDS[k]]
      if (arr && arr.length) assetCount += arr.length
    }
    if (!cover) {
      const order = ['scenes', 'props']
      for (let k = 0; k < order.length && !cover; k++) {
        const arr = assets[order[k]] || []
    ...
    // 分集是**项目内的概念**（工作台 pickShots 也按镜头自己的 episode 过滤），
    // 所以列表要给"这部剧有几集"，而不是把每集拆成一个项目。
    const epSet = Object.create(null)
    for (let i = 0; i < (doc.shots || []).length; i++) {
      const e = String((doc.shots[i] || {}).episode || '').trim()
      if (e) epSet[e] = true
    }
    let finalCount = 0
    for (let i = 0; i < clips.length; i++) if (clips[i].final) finalCount += 1
    return {
      id,
      title: meta.title || id,
      genre: meta.genre || '',
      ...
      episodeCount: Object.keys(epSet).length,
  ...
  // ───────────────────── 作业健康：心跳 / 落盘 / 看门狗 ─────────────────────
  //
  // 事故（2026-09-23，用户遇到）：某次方案阶段的大模型调用永久挂住 →
  // running 一直是 true、宿主内存里的日志从不落盘 → 界面上只有一句
  // "已有一条管线在运行"，既没有日志也没有出路，用户被永久挡住。
  // 这三件小事就是那次的整改：**作业必须始终能被看见、并且始终能被放行**。

  /** 内存里最多保留多少个已结束作业（给界面回看"刚跑完的那次"）。 */
  const JOB_KEEP = 20

  function jobAgeSec(job) { return job && job.startedAt ? Math.round((Date.now() - job.startedAt) / 1000) : 0 }
  function jobQuietSec(job) { return job && job.lastBeat ? Math.round((Date.now() - job.lastBeat) / 1000) : 0 }

  /** 当前正在跑的阶段名（给"它在干嘛"一个说法）。 */
  function jobStageLabel(job) {
    const st = (job && job.stages) || []
    for (let i = 0; i < st.length; i++) if (st[i].state === 'running') return st[i].name
    for (let i = 0; i < st.length; i++) if (st[i].state === 'pending') return '等待 ' + st[i].name
    return job && job.running ? '启动中' : ''
  }

  /**
   * 把作业日志落盘（同一作业始终覆盖同一个文件，便于边跑边看）。
   * 原实现只在作业结束时落盘 —— 于是"卡住的作业"恰恰是没有日志的那种，
   * 而它正是最需要日志的。
   */
  async function flushJobLog(job) {
    try {
      if (!job.logRel) {
        const stamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19)
        job.logRel = 'output/logs/' + job.id + '-' + stamp + '.log'
      }
      job.logFlushedLen = job.log.length
      job.logFlushedAt = Date.now()
      // **建目录走 node:fs，不走 cmd**：writeText 内部用 `cmd /c md` 建目录，
      // 而那条路要等子进程返回 —— 子进程服务一旦卡住（实测：测试里的假死子进程），
      // 连"停止作业"都会跟着挂住。日志落盘是最不该依赖别人的一件事。
      const nodeFsp = await import('node:fs/promises')
      const nodePath = await import('node:path')
      const fileAbs = abs(job.project + '\\' + job.logRel.replace(/\//g, '\\'))
      await nodeFsp.mkdir(nodePath.dirname(fileAbs), { recursive: true })
      await writeFileStamped(fileAbs, job.log)
    } catch (e) { /* 落盘失败不阻断管线 */ }
  }
   ...
   * 看门狗（每 20 秒一次）。做两件事：
   *   ① 日志增量落盘；
   *   ② 静默超过 JOB_STALE_MS 就**强制收尾**，把 running 放掉。
   * "静默"用**日志长度有没有变**来判定 —— 不用去 40 个输出点埋心跳，改一处就够，
   * 而且任何未来的死循环/挂起都自动被覆盖。
   */
  async function jobWatchdogTick() {
    const keys = Object.keys(jobs)
    for (let i = 0; i < keys.length; i++) {
      const job = jobs[keys[i]]
      if (!job.running) continue
      if (job.log.length !== job.lastLogLen) {
        job.lastLogLen = job.log.length
        job.lastBeat = Date.now()
      }
      if (job.log.length !== job.logFlushedLen) await flushJobLog(job)
      const quiet = Date.now() - (job.lastBeat || job.startedAt || Date.now())
      if (quiet <= JOB_STALE_MS) continue
      const sec = Math.round(JOB_STALE_MS / 1000)
      job.log += '\n[看门狗] 已 ' + (sec >= 60 ? Math.round(sec / 60) + ' 分钟' : sec + ' 秒')
        + '没有任何输出，判定这个作业卡死了：**已放行**，你可以重新开跑。\n'
        + '  排查建议：渲染/资产阶段先看 ComfyUI 是否还在响应（「ComfyUI」页有状态与队列）。\n'
      job.stalled = true
      try { if (job.handle) job.handle.terminate() } catch (e) { /* 可能已退出 */ }
      for (let k = 0; k < job.stages.length; k++) {
        if (job.stages[k].state === 'running') job.stages[k].state = 'failed'
      }
      job.running = false
      job.exitCode = -1
      await flushJobLog(job)
    }
  }

  /** 只保留最近的若干个已结束作业，别让内存随运行时长无限增长。 */
  function pruneJobs() {
    const keys = Object.keys(jobs)
    const done = []
    for (let i = 0; i < keys.length; i++) if (!jobs[keys[i]].running) done.push(keys[i])
    if (done.length <= JOB_KEEP) return
    done.sort((a, b) => (jobs[a].startedAt || 0) - (jobs[b].startedAt || 0))
    for (let i = 0; i < done.length - JOB_KEEP; i++) delete jobs[done[i]]
  }

  // 看门狗常驻（注册在 effect 里，插件卸载时自动清掉）
  ctx.effect(() => {
    const t = setInterval(() => {
      jobWatchdogTick().catch(() => { /* 看门狗自己绝不许把宿主带崩 */ })
    }, JOB_TICK_MS)
  ...
  function defaultParams() {
    return {
      ...
      // 空闲多少分钟后自动退出 ComfyUI（0 = 不管）。
      // 默认给 15 分钟：空转时它压着 26 GB 内存 + 21.9 GB 显存不放，
      // 不设默认值等于把这个资源黑洞留给用户自己发现。下一次渲染会自动拉起。
      idleExitMin: 15,
  ...
  function planSystem(params) {
    ...
    return [
      ...
      '    "scene": "s1",',
      '    "dialogue": [{"speaker":"角色名","text":"中文台词原文","kind":"sys|danmaku|sfx"}],   // 无台词写 []；kind 可选，只用于字幕卡，普通台词不要写',
      ...
      // ── 字幕卡（短剧"易懂 + 会玩梗"靠的是不只一行底部字幕）──
      // 这条规则是**数据侧唯一的产出方**：不写它，kind 就永远没人填，
      // 渲染器那边支持的字幕卡类型就永远是空转。
      '- **字幕卡（可选字段 `kind`）**：除底部台词外，还可以用三类"卡"来承载信息与节奏。'
        + '**宁可少一张，不要每镜都加** —— 满屏卡片会立刻显出模板味。',
      '  * `"kind":"sys"` 系统提示：把金手指/世界的**规则**钉死（左上角薄荷绿）。一集 0–2 条，只在"观众必须秒懂规则"的那一拍用。',
      '  * `"kind":"danmaku"` 弹幕：替观众说出那句吐槽（顶部小灰字）。一集 0–2 条，且必须是**观众真会这么想**的一句话，不是硬凑的梗。',
      '  * `"kind":"sfx"` 音效大字：节奏重音，2–4 个字（如「轰」「啪」）。一集 0–3 条，只给真正的重拍。',
      '  * 普通台词与旁白**一律不要写 `kind`**；`kind` 只写这三个值之一，写别的会被丢弃。',
      '',
      '## H3 提示词格式（必须严格遵守）',
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (Array.isArray(s.dialogue) && s.dialogue.length) {
        ...
        for (let k = 0; k < s.dialogue.length; k++) {
          ...
          if (!text) continue
          const one = { speaker: clean(line.speaker), text: text }
          // 字幕卡类型**必须在这一步保住**：这条通路原先只搬 speaker/text，
          // 模型或作者写进去的 kind 会在方案落地时被静默丢掉 ——
          // 于是"字幕卡"永远只有渲染器一侧、没有产出方（数据在中间就被吃了）。
          const kind = normCardKind(line.kind)
  ...
  async function callLlm(route, system, user, onDelta) {
    ...
    // ── 两道时间闸（见 LLM_IDLE_MS 的注释：没有它，一次卡住的调用会把工作台永久锁死）──
    const t0 = Date.now()
    let idleTimer = null
    let abortIdle = null
    const idleErr = new Promise((_, rej) => { abortIdle = rej })
    const bumpIdle = () => {
      if (idleTimer) clearTimeout(idleTimer)
      idleTimer = setTimeout(() => {
        abortIdle(new Error('模型调用超时：' + Math.round(LLM_IDLE_MS / 1000) + ' 秒内没有任何输出（已放弃等待）'))
      }, LLM_IDLE_MS)
    }
    bumpIdle()
    const it = stream[Symbol.asyncIterator] ? stream[Symbol.asyncIterator]() : null
    if (!it) throw new Error('模型流不可迭代')
    try {
      for (;;) {
        const nxt = await Promise.race([it.next(), idleErr])
        if (Date.now() - t0 > LLM_TOTAL_MS) {
          throw new Error('模型调用超过总时限 ' + Math.round(LLM_TOTAL_MS / 60000) + ' 分钟（已放弃等待）')
        }
        if (nxt && nxt.done) break
        const c = nxt && nxt.value
        if (!c) continue
        bumpIdle()
  ...
  async function planSource(job) {
    ...
    files.sort()
    // 分集优先：跑第 3 集时就该拿第 3 集的脚本，而不是按文件名排序的第一个
    if (ep) {
      const want = ep + '.md'
      if (files.indexOf(want) >= 0) {
        const t = await readText(job.project + '\\script\\' + want, '')
        if (t && t.trim()) return { text: t, from: 'script/' + want }
      }
    }
    if (files.length) {
    ...
    // 分集素材优先于 novel.md：一键做视频把每一集的正文单独存成 novel-<集>.md，
    // 这样"重跑第 3 集的方案"不会被后来写进去的第 5 集正文顶掉。
    if (ep) {
      const own = await readText(job.project + '\\novel-' + ep + '.md', '')
      if (own && own.trim()) return { text: own, from: 'novel-' + ep + '.md' }
    }
  ...
  async function genPlan(job) {
    ...
    // plan.json 同样备份：分集时 stampEpisode 要靠它把**别的集**的角色/场景卡并回来，
    // 只备份 shots.json 会让第二集把第一集的定妆卡从方案里洗掉。
    const oldPlan = await readText(job.project + '\\plan.json', '')
    if (oldPlan && oldPlan.trim()) {
      try { await writeText(job.project + '\\plan.json.bak', oldPlan) } catch (e) { /* 备份失败不阻断 */ }
    }
  ...
  /**
   * 给方案阶段的产物盖「集」戳：镜头打 `episode` 字段、id 加 `<集>-` 前缀，
   * 并与项目里**其它集已有的镜头合并**（一项目=一部剧，集是项目内的概念）。
   *
   * 为什么必须合并：genPlan 是整份重写 shots.json。做第二集时不合并，
   * 第一集的镜头会凭空消失 —— 第一集的成片就再也无法重渲、故事板也看不到它。
   * 旧内容从 genPlan 留的 shots.json.bak 取：它天然就是"本次覆盖前的那一份"，
   * 所以即便 AI 返修导致方案阶段跑第二遍，取到的仍是完整的上一次状态。
   */
  async function stampEpisode(job) {
    const ep = String(job.episode || '')
    if (!ep) return
    const doc = await readJson(job.project + '\\shots.json', null)
    if (!doc || !doc.shots || !doc.shots.length) return
    // 这一轮方案没有产出新镜头（跳过重写 / 复跑旧集）时不要动旧数据，
    // 否则会把别的集也重新盖一遍、甚至从陈旧的 .bak 里复制出重复镜头。
    let unstamped = 0
    for (let i = 0; i < doc.shots.length; i++) {
      if (!String((doc.shots[i] || {}).episode || '').trim()) unstamped += 1
    }
    if (!unstamped) return

    const back = await readJson(job.project + '\\shots.json.bak', null)
    const others = []
    if (back && back.shots) {
      for (let i = 0; i < back.shots.length; i++) {
        if (String(back.shots[i].episode || '').trim() !== ep) others.push(back.shots[i])
      }
    }
    let renamed = 0
    const fresh = []
    for (let i = 0; i < doc.shots.length; i++) {
      const s = Object.assign({}, doc.shots[i])
      const old = String(s.id || ('s' + (i + 1)))
      const nid = old.indexOf(ep + '-') === 0 ? old : (ep + '-' + old)
      if (nid !== old) renamed += 1
      s.id = nid
      s.episode = ep
      fresh.push(s)
    }
    ...
    // plan.json 同理：角色/场景卡是资产阶段的输入，按 id（或名字）去重取并集，
    // 否则做第二集时第一集的定妆卡会从方案里消失，"跨集同一张脸"就失去依据。
    const planDoc = await readJson(job.project + '\\plan.json', null)
    if (planDoc) {
      const backPlan = await readJson(job.project + '\\plan.json.bak', null)
      const uniq = (a, b) => {
        const out = []
        const seen = {}
        ...
        for (let i = 0; i < all.length; i++) {
          const c = all[i] || {}
          const k = String(c.id || '') || ('n:' + String(c.name || ''))
          if (seen[k]) continue
          seen[k] = true
          out.push(c)
        }
      ...
      const oldShots = []
      if (backPlan && backPlan.shots) {
        for (let i = 0; i < backPlan.shots.length; i++) {
          if (String(backPlan.shots[i].episode || '').trim() !== ep) oldShots.push(backPlan.shots[i])
        }
      }
      await writeText(job.project + '\\plan.json', JSON.stringify({
        project: planDoc.project || job.project,
        style: planDoc.style || doc.style || '',
        characters: uniq(backPlan && backPlan.characters, planDoc.characters),
        scenes: uniq(backPlan && backPlan.scenes, planDoc.scenes),
        shots: oldShots.concat(fresh),
      }, null, 2))
    }

    // project.json 登记集名：没登记就先用集号兜底（片头卡至少要有个能显示的名字）
    const meta = await readJson(job.project + '\\project.json', {})
    const eps = Object.assign({}, meta.episodes || {})
    if (!eps[ep]) {
      eps[ep] = ep
      meta.episodes = eps
      await writeText(job.project + '\\project.json', JSON.stringify(meta, null, 2))
    }

    job.log += '  集 ' + ep + '：' + fresh.length + ' 镜（id 前缀 ' + ep + '-，改 ' + renamed + ' 个）'
      + (others.length ? ' · 并入其它集 ' + others.length + ' 镜 → shots.json 共 ' + merged.length + ' 镜' : '') + '\n'
  }
  ...
  async function prepareRender(job) {
    ...
    const meta = await readJson(job.project + '\\project.json', {})
    const params = Object.assign(defaultParams(), meta.params || {})
    // 集：镜头自己的 episode 字段说了算（与 pickShots 的 range.episode 同一口径）。
    // 这一步不能省 —— 不分集就把全本镜头拿去渲染，就是"做第二集时渲了第一集的东西"。
    if (job.episode) {
      const keep = []
      for (let i = 0; i < doc.shots.length; i++) {
        if (String((doc.shots[i] || {}).episode || '').trim() === job.episode) keep.push(doc.shots[i])
      }
      if (!keep.length) {
        job.renderBlocked = '这一集（' + job.episode + '）还没有镜头：先跑方案阶段'
        job.log += '  ' + job.renderBlocked + '\n'
        return
      }
      job.log += '  集 ' + job.episode + '：' + keep.length + '/' + doc.shots.length + ' 镜\n'
      doc.shots = keep
    }
    ...
    for (let i = 0; i < p.shots.length; i++) {
      const s = Object.assign({}, p.shots[i])
        // **必须把 h3_prompt 映射成 prompt**：渲染器只认 `prompt`，plan 形状的 shots.json 里
        // 往往只有 `h3_prompt`（驱动器文档就是这么写的）。界面显示走 `prompt || h3_prompt`，
        // 所以看起来一切正常，但工作台点「渲染」时每个镜头会报 "没有 prompt"、整批 0 秒全灭。
        // 驱动器 sync 早就有这一步（两边不同构，正是审计指出的隐患）。
        if (!s.prompt && s.h3_prompt) s.prompt = s.h3_prompt
    ...
    // 分集各有自己的渲染清单（与驱动器同口径：_render-<集>.json）；
    // 同时刷新 _render.json —— 界面上的「渲染」按钮读的正是它，
    // 不刷新就会拿旧集的清单去渲，而界面上看不出任何异常。
    const docRel = job.episode ? ('_render-' + job.episode + '.json') : '_render.json'
    await writeText(job.project + '\\' + docRel, JSON.stringify(renderDoc, null, 2))
    if (job.episode) await writeText(job.project + '\\_render.json', JSON.stringify(renderDoc, null, 2))
    job.shotsArg = out + '\\' + docRel
  ...
  async function runPipeline(job) {
    ...
    // 盖集戳：这一集新产出的镜头打上 episode 与 id 前缀，并与项目里其它集合并。
    // 放在方案之后、资产之前 —— 资产定妆与参考图解析都要看到带集号的镜头。
    await stampEpisode(job)
    ...
    job.running = false
    job.exitCode = 0
    job.logFile = await saveJobLog(job.project, 'pipeline', job.log)
    // 一整条管线跑完才释放显存；有别的任务接手时 ensureComfy 会取消它
    scheduleFree(comfyUrlOf(Object.assign(defaultParams(), (await readJson(job.project + '\\project.json', {})).params || {})), 45, '管线完成')
  }
  ...
  async function qcProject(pid, opts) {
    const o = opts || {}
    // 分集质检只查这一集的镜头（同样的前缀口径：镜头 id 带 `<集>-`）——
    // 不裁的话，第二集的质检会为第一集的镜头反复报合格/不合格，把真实问题淹掉。
    const ep = String(o.episode || '').trim()
    const clips = clipsOf(await listDir(pid))
    const names = []
    // 抽卡备选（<sid>_take<k>）不参与合成与质检 —— 否则成片里会出现
    // 同一镜的好几个版本，质检也会为备选白跑 N 倍
    for (let i = 0; i < clips.length; i++) {
      if (clips[i].final || clips[i].take) continue
      if (ep && clips[i].name.indexOf(ep + '-') !== 0) continue
      names.push(clips[i].name)
    }
    names.sort()
    const reports = []
    for (let i = 0; i < names.length; i++) reports.push(await qcOneClip(pid, names[i], o))
    let failed = 0
    let warned = 0
  ...
  function buildAss(shots, starts, opts) {
    ...
    let n = 0
    // 每镜尾部被下一镜吃掉的秒数：叠化时是转场时长 F，硬切是 0。
    // **不扣掉它就会出事故**：字幕窗口原本铺满整镜时长，而叠化时下一镜提前 F 秒开始，
    // 于是每处转场都有 F 秒两条字幕一起压在屏幕上（实测 15 镜 = 14 处重叠、每处 0.44s），
    // 观感是"字和画都在重影"，直接被读成"镜头前后不连贯"。
    const tailTrim = Number(o.tailTrim) || 0
    // 字幕淡入淡出：知识库漫剧字幕规则要求文字"淡入"而不是硬切。
    // 只在显式传 0 时关闭（o.subtitleFadeMs === 0）。
    const fadeMs = o.subtitleFadeMs === undefined ? 140 : (Number(o.subtitleFadeMs) || 0)
    for (let i = 0; i < shots.length; i++) {
      ...
      const dur = (s._dur || 0)
      const avail = dur - (tailTrim && i < shots.length - 1 ? tailTrim : 0)
      if (avail <= 0.05) continue
      const each = avail / dlg.length
      for (let k = 0; k < dlg.length; k++) {
        const d = dlg[k] || {}
        const txt = String(d.text || '').trim()
        if (!txt) continue
        const speakerName = String(d.speaker || '')
        const isNarr = /旁白|narrator|voiceover/i.test(speakerName)
        // **角色台词带说话人前缀**：观众要能知道是谁在说（旁白不加，它本来就没有"人"）。
        // 前缀并入折行计算，免得顶出画面。
        const label = (isNarr || !speakerName) ? '' : (speakerName + '：')
        // 留 0.06s 空隙，避免相邻字幕首尾相接显得粘连
        const t0 = start + each * k
        const t1 = start + each * (k + 1) - 0.06
        if (t1 - t0 < 0.2) continue
        // 字幕卡（sys / danmaku / sfx）走行内覆盖标签：位置与字号都不同，
        // 普通台词/旁白仍是底部一行 + 说话人前缀。
        const kind = normCardKind(d.kind)
        const card = kind ? assCard(kind, txt, { width: W, height: H, size: size, fadeMs: fadeMs }) : null
        lines.push('Dialogue: 0,' + assTime(t0) + ',' + assTime(t1) + ','
          + (card ? card.style : (isNarr ? '旁白' : '对白')) + ',' + speakerName + ',0,0,0,,'
          // 字幕淡入淡出（知识库漫剧字幕规则：文字要淡入而不是硬切）。
          // 140ms 是"看得出是淡入、不觉得慢"的量；o.subtitleFadeMs = 0 可关。
          + (card ? card.text
            : ((fadeMs ? '{\\fad(' + fadeMs + ',' + fadeMs + ')}' : '') + wrapAssText(label + txt, maxUnits))))
        n += 1
      }
    }
  ...
  async function composeFinal(pid, opts) {
    const o = opts || {}
    // 集号先算出来 —— 它决定"哪些镜头算这一集的"以及成片叫什么名字。
    const ep = String(o.episode || '').trim()
    if (ep && !/^[A-Za-z0-9_-]{1,20}$/.test(ep)) return { ok: false, error: '集号非法：' + ep }
    const clips = clipsOf(await listDir(pid))
    const names = []
    // 抽卡备选（<sid>_take<k>）不参与合成与质检 —— 否则成片里会出现
    // 同一镜的好几个版本，质检也会为备选白跑 N 倍
    // 分集合成只取这一集的镜头：镜头 id 带 `<集>-` 前缀（ep01-s01.mp4），
    // 不按前缀裁就会把别的集一起拼进这一集里 —— 而画面看着完全正常，最难发现。
    for (let i = 0; i < clips.length; i++) {
      if (clips[i].final || clips[i].take) continue
      if (ep && clips[i].name.indexOf(ep + '-') !== 0) continue
      names.push(clips[i].name)
    ...
    names.sort()
    if (!names.length) {
      return { ok: false, error: ep ? ('这一集（' + ep + '）还没有渲好的镜头') : '没有可合成的镜头（成片不计入）' }
    }
    const dir = abs(pid)
    // 分集成片各自一份：成片-<集>.mp4。驱动器的分集合成就是这个命名，
    // 两边不一致会导致"回传的成片和界面认的不是同一个文件"。
    const outName = ep ? ('成片-' + ep + '.mp4') : '成片.mp4'
    const assRel = 'output/final' + (ep ? '-' + ep : '') + '.ass'
    const target = abs(pid + '\\' + outName)
    const FF = await exePath('ffmpeg')
    ...
    if (transition === 'cut' || names.length === 1) {
      ...
      // 响度归一必须**并进滤镜图**：这条分支的音轨已经过 filter_complex，
      // 再用 `-af loudnorm` 会撞 "Simple and complex filtering cannot be used
      // together for the same stream" —— 叠化 + 字幕/响度归一必然失败。
      parts.push('[' + aLabel + ']' + af + '[aout]')
      aLabel = 'aout'
  ...
  /**
   * 归还 ComfyUI 占着的显存与内存（POST /free）。
   *
   * 为什么必须做：实测空转时一个 python 进程压着 26 GB 内存 + 21.9 GB 显存不放
   * （GPU 利用率只有 5%），系统提交内存顶到 87.9 GB 上限。而 /free 实测
   * 显存 21.9 GB → 0.99 GB、进程内存 26 GB → 0.81 GB，HTTP 200。
   *
   * 时机很关键：**只在整批任务结束时调**。每镜之间调会让下一镜重新加载权重，
   * 白白多花几十秒 —— 省钱的地方不在那里。
   */
  async function comfyFree(url, reason) {
    const ctl = new AbortController()
    const timer = setTimeout(() => ctl.abort(), 20000)
    try {
      const res = await fetch(comfyUrlOf({ comfyUrl: url }) + '/free', {
        method: 'POST',
      ...
      if (lg && typeof lg.info === 'function') {
        lg.info('manju-studio: 归还 ComfyUI 显存/内存（' + (reason || '任务结束') + '，HTTP ' + res.status + '）')
      }
  ...
  /**
   * 延迟归还：任务刚结束时先等一小会儿再释放。
   * 连渲/连跑是常态（返修、抽卡、下一批），立刻释放会让下一次任务重新加载权重；
   * 而真有新任务开始时 ensureComfy 会把这个定时器取消掉。两头都不亏。
   */
  let freeTimer = null
  function cancelScheduledFree() {
    if (freeTimer) { clearTimeout(freeTimer); freeTimer = null }
  }
  function scheduleFree(url, sec, reason) {
    cancelScheduledFree()
    const wait = Math.max(5, Number(sec) || 45)
    freeTimer = setTimeout(async () => {
      freeTimer = null
      const keys = Object.keys(jobs)
      for (let i = 0; i < keys.length; i++) if (jobs[keys[i]].running) return
      await comfyFree(url, (reason || '批次结束') + '，等待 ' + wait + 's 无新任务')
    }, wait * 1000)
  }
  /**
   * 空闲自动退出。
  ...
  async function ensureComfy(job, params) {
    ...
    // 有新任务要跑 → 撤掉上次排的延迟释放，别把刚热起来的权重再卸掉
    cancelScheduledFree()
  ...
  /**
   * 项目封面 —— 读 `project.json` 的 `cover` 字段，兜底认项目根下的 `cover.png`。
   */
  async function coverOf(id) {
    const meta = await readJson(id + '\\project.json', {})
    const rel = String(meta.cover || '').trim()
    if (rel) {
      const st = await fs.stat(await fs.resolve(abs(id + '\\' + rel.replace(/\//g, '\\'))))
      if (st && st.type === 'file') return normRel(id, rel)
    }
    const entries = await listDir(id)
    for (let i = 0; i < entries.length; i++) {
      const e = entries[i]
      if (e.type === 'file' && /^cover\.(png|jpe?g|webp)$/i.test(e.name)) return e.name
    }
    return ''
  }

  /**
   * 封面提示词 —— **要的是"这一集的门面"，不是人物照**。
   *
   * 为什么必须独立生成封面（用户明确要求）：
   *   * 定妆照是 Ref2VA 的**身份锚点**，天生正面半身、中性背景、表情克制 ——
   *     拿它当封面等于把海报做成证件照；
   *   * 它是"某一个角色"的脸，而封面要代表**整部片子**（世界观/基调/场面）。
   * 所以封面一律环境优先、人物只能是远处剪影（不出现可辨五官），并留出上部留白给标题
   * （标题由界面用 .mj-covertitle 叠在图上，所以图里**绝不能带字**）。
   */
  function coverBrief(meta, params, plan) {
    const style = effectiveStyle(params)
    const genre = String(meta.genre || '').trim()
    const syn = String(meta.synopsis || '').trim()
    const custom = String(meta.coverPrompt || '').trim()
    const parts = []
    // **绝不把片名写进提示词**：写进去模型就会把字画在图上，而 H3/Krea 画汉字必歪，
    // 况且界面本来就会把标题叠在缩略图上（.mj-covertitle）—— 图里再带字就是双标题 + 错字。
    parts.push('EPISODE KEY ART — one single iconic wide cinematic image serving as the cover of a Chinese 2.5D '
      + 'animated series episode.')
    if (genre) parts.push('Genre: ' + genre + '.')
    if (syn) parts.push('Story premise: ' + syn)
    // 取作品自己的场景当取景对象 —— **只取一个**。
    // 实测教训：给两个场景，模型会画成**四宫格拼贴**（一眼就是模板货），光写 no collage 压不住。
    const scenes = []
    const sc = (plan && plan.scenes) || []
    for (let i = 0; i < sc.length && scenes.length < 1; i++) {
      const d = String((sc[i] && sc[i].description) || '').trim()
      if (d) scenes.push(d)
    }
    if (scenes.length) parts.push('SETTING — drawn from this series\' own scene design: ' + scenes[0])
    if (custom) parts.push('Additional art direction: ' + custom)
    parts.push('COMPOSITION: ONE single continuous photographic frame — a single environment-first wide shot that '
      + 'captures the mood of the whole episode — a strong sense of place, dramatic light and atmosphere, deep depth '
      + 'staging, clear silhouette and colour separation, rich incidental detail. A small distant figure may appear only '
      + 'for scale, seen from behind or as an unlit silhouette, never showing a readable face. Leave the upper third calm '
      + 'and uncluttered so an episode title can be overlaid later.')
    parts.push('MUST NOT: no collage, no split screen, no panels, no insets, no divided sections, no borders, no frames '
      + 'within frames, no multiple views of different places, no character portrait, no close-up face, no front-facing '
      + 'bust, no cast line-up, no beauty close-up, no market street, no shop fronts, no hanging signboards, no plaques, '
      + 'no banners, no scrolls, no text, no lettering, no calligraphy, no glyphs, no symbols that look like writing, '
      + 'no logo, no watermark, no UI.'
      + ' The image must read as the poster of a PLACE and a MOMENT, not as a portrait of a person and not as a set of thumbnails.')
    if (style) parts.push('RENDER STYLE (surface only, never changes features): ' + style)
    return parts.join('\n\n')
  }

  /**
   * 生成一张独立封面，落到项目根 `cover.png`，并把路径记进 project.json。
   * 尺寸取 1344×768：成品列表缩略图是 56×32（≈16:9），竖版海报会被裁得只剩中间一条。
   */
  async function genCover(pid, params, job) {
    const meta = await readJson(pid + '\\project.json', {})
    if (!String(meta.title || '').trim()) return { ok: false, error: '项目还没有标题，先填作品信息再生成封面' }
    const recipe = imageRecipe(params)
    const url = comfyUrlOf(params)
    const prompt = coverBrief(meta, params, await readJson(pid + '\\plan.json', null))
    const w = 1344
    const h = 768
    const seed = 1000 + Math.floor(Math.random() * 900000)
    const graph = buildImageGraph(recipe, prompt, w, h, seed)
    if (job) job.log += '    生成封面「' + meta.title + '」' + w + '×' + h + ' seed=' + seed + ' …\n'
    const run = await comfyRunGraph(url, graph, (params.genTimeoutSec || 600) * 1000, (note) => {
      if (job) job.log += '      ' + note + '\n'
    })
    if (!run.ok) return { ok: false, error: run.error }
    ...
    try {
      const fsp = await import('node:fs/promises')
      await fsp.copyFile(src, abs(pid + '\\' + rel))
    } catch (e) {
      return { ok: false, error: '封面搬运失败：' + String((e && e.message) || e) }
    }
    meta.cover = rel
    meta.coverSeed = seed
    meta.coverAt = new Date().toISOString()
    meta.coverPromptFinal = prompt
    await writeText(pid + '\\project.json', JSON.stringify(meta, null, 2))
    if (job) job.log += '      ✓ ' + rel + '（独立封面，非人物定妆照）\n'
    return { ok: true, rel: rel, seed: seed, width: w, height: h }
  }
  ...
  async function genAssets(job, only) {
    ...
    if (!todo.length) {
      ...
      // 素材齐 ≠ 封面有：封面是**独立一张图**，得单独补（且不能拿定妆照顶替）
      const cov = await ensureCover(job, params)
      if (cov.made) return { ok: true, made: 1, failed: [], total: 1 }
      if (cov.error) return { ok: true, made: 0, failed: [['封面', cov.error]], total: 1 }
    ...
    // 素材补齐的顺带把封面也补上：封面是**独立一张图**，不能拿定妆照顶替，
    // 所以它必须有自己的生成步骤（用户明确要求）。
    if (!job.stop) {
      const cov = await ensureCover(job, params)
      if (cov.error) failed.push({ kind: 'cover', name: '封面', error: cov.error })
    }
    return { ok: failed.length === 0, made: made.length, failed: failed, total: todo.length }
  }

  /**
   * 确保项目有一张独立封面：已有就跳过，没有就生成。
   * 返回 { made, skipped?, error? } —— 调用方据此决定是否记进失败清单。
   */
  async function ensureCover(job, params) {
    if (await coverOf(job.project)) {
      job.log += '  封面已有，跳过。\n'
      return { made: false, skipped: true }
    }
    const meta = await readJson(job.project + '\\project.json', {})
    if (!String(meta.title || '').trim()) {
      job.log += '  项目没有标题，跳过封面（封面按标题/题材/简介生成）。\n'
      return { made: false, skipped: true }
    }
    const cst = await ensureComfy(job, params)
    if (!cst.ok) return { made: false, error: cst.error }
    const r = await genCover(job.project, params, job)
    if (r.ok) return { made: true }
    return { made: false, error: r.error }
  }
  ...
  // ═══════════ 小说工作区（D:\Ai\小说）：作品管理 ═══════════
  //
  // 与下面「小说库」的区别（两者不是一回事，别再合并）：
  //   * 下面那套面向**文件**：dir.list 浏览目录、novel.scan 扫出可导入的 .txt/.md、
  //     novel.import 把某一段正文导进当前项目 —— 入口是"选一个文件"。
  //   * 这一套面向**作品**：小说工作区里的一部作品是一个结构化目录 ——
  //     立项.json（书名/题材/卷规划/美术方向）+ 正文/<卷>/第NNN章_*.md + 设定集/ + 全本/ + 封面/。
  //     要能列出作品、看内容，并且**从作品一键起步做成漫剧**。

  /** 小说工作区根目录的内存缓存。封面缩略图会频繁问它，每次请求都读盘没必要。 */
  let novelRootMemo = null

  /**
   * 小说工作区根目录：配置优先，缺失/损坏则退回默认（D:\Ai\小说）。
   * 配置坏了不能让整页打不开 —— 退回默认是最不容易出事的降级。
   */
  async function novelRootPath() {
    if (novelRootMemo) return novelRootMemo
    let p = ''
    try {
      const cfg = await readJson(STUDIO_CFG, null)
      if (cfg && typeof cfg.novelRoot === 'string') p = cfg.novelRoot.trim()
    } catch (e) { /* 配置坏了退回默认 */ }
    if (!p) p = NOVEL_ROOT_DEFAULT
    novelRootMemo = p.replace(/[\\/]+$/, '')
    return novelRootMemo
  }

  /** 改小说工作区目录（落盘，重启后仍在）。 */
  async function setNovelRoot(p) {
    const v = String(p || '').trim().replace(/[\\/]+$/, '')
    if (!/^[A-Za-z]:\\/.test(v)) throw new Error('请填绝对路径，例如 D:\\Ai\\小说')
    const st = await fs.stat(await fs.resolve(v))
    if (!st || st.type !== 'directory') throw new Error('目录不存在：' + v)
    novelRootMemo = v
    const cfg = (await readJson(STUDIO_CFG, null)) || {}
    cfg.novelRoot = v
    await writeText(STUDIO_CFG, JSON.stringify(cfg, null, 2))
    return v
  }

  /** 作品目录名清单（书名就是目录名，中文；排除隐藏/下划线目录与非法名）。 */
  async function workDirs() {
    const root = await novelRootPath()
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      if (e.type !== 'directory') continue
      if (e.name.charAt(0) === '.' || e.name.charAt(0) === '_') continue
      if (!WORK_RE.test(e.name) || e.name.indexOf('..') >= 0) continue
      out.push(e.name)
    }
    out.sort()
    return out
  ...
  /** 章标题：文件名就是标题（第001章_八文一斤.md → 第001章 八文一斤），不必读正文。 */
  function chapterTitleOf(name) {
    return String(name || '').replace(/\.(md|txt)$/i, '').replace(/_/g, ' ').trim()
  }

  /**
   * 正文的卷与章清单：正文/<卷>/第NNN章_*.md。
   * 也兼容"正文直接平铺在 正文/ 下"的作品（用一个空卷名兜住）。
   */
  async function workVolumes(dir) {
    const src = dir + '\\正文'
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      if (e.type === 'file') {
        if (TEXT_EXT.test(e.name)) {
          flat.push({ rel: '正文/' + e.name, name: e.name, title: chapterTitleOf(e.name), size: e.size || 0, chars: Math.max(1, Math.round((e.size || 0) / 3)) })
        }
        continue
      }
      if (e.name.charAt(0) === '.' || e.name.charAt(0) === '_') continue
      const files = await listDir(src + '\\' + e.name)
      const chapters = []
      for (let k = 0; k < files.length; k++) {
        const f = files[k]
        if (f.type !== 'file' || !TEXT_EXT.test(f.name)) continue
        chapters.push({
          rel: '正文/' + e.name + '/' + f.name,
          name: f.name,
          title: chapterTitleOf(f.name),
          size: f.size || 0,
          chars: Math.max(1, Math.round((f.size || 0) / 3)),
        })
      }
      chapters.sort((a, b) => (a.name < b.name ? -1 : 1))
      if (chapters.length) {
        let chars = 0
        for (let k = 0; k < chapters.length; k++) chars += chapters[k].chars
        vols.push({ vol: e.name, chapters: chapters, chars: chars })
      }
    }
    flat.sort((a, b) => (a.name < b.name ? -1 : 1))
    if (flat.length) {
      let chars = 0
      for (let k = 0; k < flat.length; k++) chars += flat[k].chars
      vols.push({ vol: '', chapters: flat, chars: chars })
    }
    vols.sort((a, b) => (a.vol < b.vol ? -1 : a.vol > b.vol ? 1 : 0))
    return vols
  }

  /** 全本文件（最大的那个 .md/.txt）。 */
  async function workFull(dir) {
    const entries = await listDir(dir + '\\全本')
    let best = null
    for (let i = 0; i < entries.length; i++) {
      const e = entries[i]
      if (e.type !== 'file' || !TEXT_EXT.test(e.name)) continue
      if (!best || (e.size || 0) > (best.size || 0)) best = e
    }
    if (!best) return null
    return { rel: '全本/' + best.name, name: best.name, size: best.size || 0, chars: Math.max(1, Math.round((best.size || 0) / 3)) }
  }

  /** 设定集文件清单。 */
  async function workSettings(dir) {
    const entries = await listDir(dir + '\\设定集')
    const out = []
    for (let i = 0; i < entries.length; i++) {
      const e = entries[i]
      if (e.type === 'file' && TEXT_EXT.test(e.name)) out.push({ name: e.name, size: e.size || 0 })
    }
  ...
  /**
   * 封面：**只认成品** 封面.png/jpg/webp。
   * 候选（封面_候选*.png / 封面_v*.png）是生成中间产物，拿它们当封面会把半成品露出去。
   */
  async function workCover(dir) {
    const entries = await listDir(dir + '\\封面')
    let hit = ''
    let cand = ''
    for (let i = 0; i < entries.length; i++) {
      const e = entries[i]
      if (e.type !== 'file' || !/\.(png|jpe?g|webp)$/i.test(e.name)) continue
      const base = e.name.replace(/\.(png|jpe?g|webp)$/i, '')
      if (base === '封面') { hit = '封面/' + e.name; break }
      if (!cand && base.indexOf('封面') === 0) cand = '封面/' + e.name
    }
    if (hit) return hit
    const rootEntries = await listDir(dir)
    for (let i = 0; i < rootEntries.length; i++) {
      const e = rootEntries[i]
      if (e.type === 'file' && /^封面\.(png|jpe?g|webp)$/i.test(e.name)) return e.name
    }
    return cand
  }

  /** 作品摘要（列表用）。只读小文件与目录，正文按需再读。 */
  async function workSummary(work) {
    const root = await novelRootPath()
    const dir = root + '\\' + work
    const meta = await readJson(dir + '\\立项.json', null)
    const vols = await workVolumes(dir)
    let chapterCount = 0
    let chars = 0
    for (let i = 0; i < vols.length; i++) {
      chapterCount += vols[i].chapters.length
      chars += vols[i].chars
    }
    const full = await workFull(dir)
    const settings = await workSettings(dir)
    const cover = await workCover(dir)
    const g = (k) => (meta && meta[k] !== undefined && meta[k] !== null ? String(meta[k]) : '')
    return {
      work: work,
      dir: dir,
      title: g('书名') || work,
      ...
      hook: g('高概念'),
      logline: g('一句话卖点'),
      protagonist: g('主角形态') || g('protagonist'),
      art: g('美术方向'),
      hasMeta: !!meta,
      volCount: vols.length,
      vols: vols.map((v) => ({ vol: v.vol, chapters: v.chapters.length, chars: v.chars })),
      chapterCount: chapterCount,
      chars: chars,
      full: full ? { rel: full.rel, name: full.name, chars: full.chars } : null,
      settings: settings,
      cover: cover,
    }
  }
  ...
  async function workDetail(work) {
    const root = await novelRootPath()
    const dir = root + '\\' + work
    const s = await workSummary(work)
    const vols = await workVolumes(dir)
    const meta = await readJson(dir + '\\立项.json', null)
    return Object.assign({}, s, { volList: vols, meta: meta || null })
  }

  /** 读作品内一个文本文件（有界）。路径不允许逃出作品目录。 */
  async function readWorkText(dir, rel, maxChars) {
    const clean = String(rel || '').replace(/\\/g, '/').replace(/^\/+/, '')
    if (!clean || clean.indexOf('..') >= 0 || !/^[^:*?"<>|]+$/.test(clean)) return { error: '路径非法' }
    if (!TEXT_EXT.test(clean)) return { error: '只读文本文件（.md / .txt）' }
    const full = dir + '\\' + clean.replace(/\//g, '\\')
    if (full.toLowerCase().indexOf((dir + '\\').toLowerCase()) !== 0) return { error: '路径越界' }
    const text = await readText(full, null)
    if (text === null || text === undefined) return { error: '读不到：' + clean }
    const max = Number(maxChars) > 0 ? Number(maxChars) : 20000
    return {
      rel: clean,
      chars: text.length,
  ...
  /**
   * 把「素材范围」解析成有序章节清单 + 拼好的正文，并按 PLAN_INPUT_MAX 截断。
   *
   * 为什么一定要截断：方案阶段（genPlan）只把正文的前 12000 字喂给模型。
   * 让用户选"全本"却悄悄只做前 8 章，是把系统的局限伪装成作品的局限 ——
   * 所以这里把截断**明确算出来并回报**，界面上直接显示"进入分镜的字数 / 原作总字数"。
   *
   * scope：
   *   { kind: 'chapter', vol?, chapter?, count? }  某章起，往后 count 章（默认 1）
   *   { kind: 'volume', vol }                      整卷
   *   { kind: 'full' }                             全本（优先用 全本/，没有则按章拼）
   */
  async function workScope(dir, scope) {
    const s = scope || {}
    const kind = String(s.kind || 'chapter')
    const vols = await workVolumes(dir)
    const all = []
    for (let i = 0; i < vols.length; i++) {
      for (let k = 0; k < vols[i].chapters.length; k++) {
        all.push(Object.assign({ vol: vols[i].vol }, vols[i].chapters[k]))
      }
    }
    let picked = []
    let label = ''
    if (kind === 'full') {
      ...
      const want = String(s.vol || '')
      for (let i = 0; i < vols.length; i++) {
        if (vols[i].vol === want) {
          for (let k = 0; k < vols[i].chapters.length; k++) picked.push(Object.assign({ vol: vols[i].vol }, vols[i].chapters[k]))
        }
      }
      if (!picked.length) return { error: '找不到这一卷：' + want }
      label = want
    } else {
      let start = -1
      ...
      if (key) {
        for (let i = 0; i < all.length; i++) {
          if (all[i].rel === key || all[i].name === key || all[i].title === key) { start = i; break }
        }
        // 也接受"第001章"这种前缀匹配（用户不会去抄完整文件名）
        if (start < 0) {
          for (let i = 0; i < all.length; i++) {
            if (key && (all[i].name.indexOf(key) === 0 || all[i].title.indexOf(key) === 0)) { start = i; break }
          }
        }
      }
      if (start < 0) start = 0
      const n = Math.max(1, Math.min(30, Number(s.count) || 1))
      picked = all.slice(start, start + n)
      label = picked.length ? picked[0].title : ''
    }
    if (!picked.length) return { error: '选定范围里没有正文' }

    // 全本优先读 全本/ 那个文件（技能产出的是合并稿），其余按章拼
    let body = ''
    if (kind === 'full') {
      const full = await workFull(dir)
      if (full) body = await readText(dir + '\\' + full.rel.replace(/\//g, '\\'), '')
    }
    const readParts = []
    if (!body) {
      for (let i = 0; i < picked.length; i++) {
        const t = await readText(dir + '\\' + picked[i].rel.replace(/\//g, '\\'), '')
        if (!t) continue
        readParts.push({ rel: picked[i].rel, title: picked[i].title, text: t })
        body += (body ? '\n\n' : '') + t
      }
    } else {
      // 读的是全本：章清单仍按全本覆盖范围内的章标记（便于溯源）
      for (let i = 0; i < picked.length; i++) readParts.push({ rel: picked[i].rel, title: picked[i].title, text: '' })
    }
    const totalChars = body.length
    let text = body
    let truncated = false
    if (text.length > PLAN_INPUT_MAX) {
      text = text.slice(0, PLAN_INPUT_MAX)
      // 尽量切在段落边界，别把一句话劈成两半
      const cut = text.lastIndexOf('\n\n')
      if (cut > PLAN_INPUT_MAX * 0.6) text = text.slice(0, cut)
      truncated = true
    }
    return {
      kind: kind,
      label: label,
      chapters: readParts.map((p) => ({ rel: p.rel, title: p.title })),
      chapterCount: readParts.length,
      volCount: (function () { const m = {}; for (let i = 0; i < picked.length; i++) m[picked[i].vol] = true; return Object.keys(m).length })(),
      text: text,
      chars: text.length,
      totalChars: totalChars,
      truncated: truncated,
      maxChars: PLAN_INPUT_MAX,
    }
  }

  /** 作品名 → 一个合法的项目 id 兜底（书名是中文，项目 id 只能 ASCII）。 */
  function slugId(title) {
    const ascii = String(title || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 24)
    if (ascii.length >= 3) return ascii
    let h = 2166136261
    const s = String(title || 'novel')
    for (let i = 0; i < s.length; i++) { h ^= s.charCodeAt(i); h = (h * 16777619) >>> 0 }
    return 'manju-' + h.toString(36).slice(0, 6)
  }

  /**
   * 自动排下一个集号：ep01 / ep02 …（已占用的跳过）。
   *
   * 有了它，"再做一集"才真的是一键 —— 用户不必自己去数项目里已经有几集、
   * 也不必记住集号的格式；而重复覆盖集的风险也一并消失了。
   */
  function nextEpisodeId(eps) {
    const used = eps || {}
    for (let i = 1; i < 1000; i++) {
      const id = 'ep' + (i < 10 ? '0' + i : i)
   ...
   * 建项目骨架 —— **项目里唯一的一处建项目实现**。
   *
   * 为什么必须只有一处：一键做视频也要建项目。两份实现迟早有一份忘了写
   * assets.json / shots.json，而那两样缺一个，资产阶段与故事板当场报错。
   *
   * 只做"建骨架"，不碰正文（novel.md 留空）—— 正文由调用方负责，
   * 因为不同入口的来源完全不同（手输路径 / 小说工作区的某一章）。
   */
  async function createProjectDir(fields) {
    const f = fields || {}
    const id = String(f.id || '').trim()
    if (!ID_RE.test(id)) throw new Error('项目名只能用字母/数字/下划线/连字符，1-40 字符')
    if (await exists(id + '\\project.json')) throw new Error('项目已存在：' + id)
    ...
    if (f.series) meta.series = String(f.series)
    if (f.episodes && Object.keys(f.episodes).length) meta.episodes = f.episodes
    if (f.targetSec) meta.targetSec = f.targetSec
    if (f.params) meta.params = f.params
    if (f.origin) meta.origin = f.origin
  ...
  const handlers = {
    ...
    /**
     * 永久删除项目 —— **不可恢复**，与 `remove`（移进 .trash）是两件事。
     *
     * 为什么要有：一个项目目录里可能有几十个 mp4（一部成片就 38 MB），
     * 软删除占的空间一分不少。用户明确要"删干净"时就该给这条路径，
     * 而不是让他自己去文件管理器里翻 —— 但必须把不可恢复写在脸上：
     *   * 只允许删 ROOT 的**直接子目录**（双保险，绝不许往上跑）；
     *   * 有任务正在跑这个项目时拒删（正写着文件会被删出半个目录）。
     */
    async 'purge'(a) {
      const id = String((a && a.id) || '')
      if (!ID_RE.test(id)) return { error: '项目 id 非法' }
      ...
      for (let i = 0; i < keys.length; i++) {
        const j = jobs[keys[i]]
        if (j && j.running && String(j.project) === id) {
          return { error: '该项目还有任务在跑（' + j.id + '），先停止再删' }
        }
      }
      const fsp = await import('node:fs/promises')
      const npath = await import('node:path')
      const dir = abs(id)
      let st = null
      try { st = await fsp.stat(dir) } catch (e) { st = null }
      if (!st || !st.isDirectory()) return { error: '项目不存在：' + id }
      const rootResolved = npath.resolve(ROOT)
      const target = npath.resolve(dir)
      if (npath.dirname(target) !== rootResolved) {
        return { error: '拒绝删除：目标不是项目根目录的直接子目录' }
      }
      let files = 0
      let bytes = 0
      const walk = async (p) => {
        let es = []
        try { es = await fsp.readdir(p, { withFileTypes: true }) } catch (e) { return }
        for (let i = 0; i < es.length; i++) {
          const one = npath.join(p, es[i].name)
          if (es[i].isDirectory()) { await walk(one); continue }
          files += 1
          try { bytes += (await fsp.stat(one)).size } catch (e) { /* 算不出来就算了 */ }
        }
      }
      await walk(target)
      try {
        ...
        return { error: '删除失败：' + String((e && e.message) || e) }
      }
      const left = await readJson(id + '\\project.json', null)
      if (left) return { error: '删除后仍能读到 project.json，未删干净' }
      return { ok: true, id: id, files: files, bytes: bytes }
    },
    ...
    async create(a) {
      try {
        await createProjectDir(a || {})
      } catch (e) {
        return { error: String((e && e.message) || e) }
      }
      return { ok: true, id: String((a && a.id) || '').trim() }
    },
    ...
    async renderStart(a) {
      ...
      // **优先用 _render.json**：shots.json 是"方案形状"（只有提示词与 characters/scene 引用），
      // 参考图是在 prepareRender 里才解析成真实路径的。直接拿 shots.json 渲染，
      // 会静默退化成 t2v —— 角色锁脸全部失效，而界面上看不出任何异常。
      let shotsArg = abs(id) + '\\shots.json'
      if (await exists(id + '\\_render.json')) shotsArg = abs(id) + '\\_render.json'
      let handle
      try {
        handle = subprocess.spawn({
          argv: manjuArgv(['render', '--shots', shotsArg, '--out', abs(id)]),
          cwd: abs(id),
          stdio: { stdin: 'ignore', stdout: { maxBytes: 2097152 }, stderr: { maxBytes: 2097152 } },
          graceMs: 6000,
        })
      ...
      const nowR = Date.now()
      const job = {
        id: jobId, project: id, kind: 'render', handle, running: true, exitCode: null,
        offOut: 0, offErr: 0, log: '',
        // 作业健康字段：看门狗与 jobs.list 都要用（少了它们，渲染中的作业会被误判卡死）
        startedAt: nowR, lastBeat: nowR, lastLogLen: 0, logFlushedLen: -1, stalled: false, logRel: '',
        mode: 'render', stages: STAGES.map((s) => ({ key: s.key, name: s.name, state: 'pending', note: '' })),
      }
      jobs[jobId] = job
      handle.done
        ...
        .then(async () => {
          // 日志落盘 + 延迟归还显存：两个都是"任务结束后该做、但很容易忘"的事
          job.logFile = await saveJobLog(id, 'render', job.log)
          const meta = await readJson(id + '\\project.json', {})
          scheduleFree(comfyUrlOf(Object.assign(defaultParams(), meta.params || {})), 45, '渲染结束')
        })
    ...
    async renderPoll(a) {
      ...
      // **同时累积到 job.log**：原实现只把日志流给界面，job.log 一直是空的 ——
      // 于是 ① 看门狗会把"渲染中但没写日志"误判成卡死；② 增量落盘没有内容可写，
      // 「渲染」按钮跑起来的日志要等结束才看得到。两件事都由这一行解决。
      if (chunk) {
        job.log += chunk
        if (job.log.length > 2097152) job.log = job.log.slice(job.log.length - 2097152)
      }
    ...
    // ── ComfyUI 启动管理 ──
    /** 手动归还显存/内存：给「ComfyUI」页一个按钮，也方便脚本化调用。 */
    async 'comfy.free'(a) {
      const id = String((a && a.id) || '')
      ...
      cancelScheduledFree()
      const before = await comfyVram()
      const r = await comfyFree(url, '手动释放')
      await sleep(1200)
      const after = await comfyVram()
      return Object.assign({ before: before, after: after }, r)
    },

    /** 日志清单：项目 output/logs 下的 .log，新的在前。 */
    async 'logs.list'(a) {
      const id = String((a && a.id) || '')
      if (!ID_RE.test(id)) return { error: '项目 id 非法' }
      const entries = await listDir(id + '\\output\\logs')
      const out = []
      const fsp = await import('node:fs/promises')
      const npath = await import('node:path')
      for (let i = 0; i < entries.length; i++) {
        const e = entries[i]
        if (e.type !== 'file' || !/\.log$/i.test(e.name)) continue
        let mtime = 0
        try { mtime = (await fsp.stat(npath.join(abs(id), 'output', 'logs', e.name))).mtimeMs } catch (err) { /* 读不到就算了 */ }
        out.push({ name: e.name, size: e.size || 0, mtime: mtime })
      }
      out.sort((x, y) => (y.mtime - x.mtime) || (x.name < y.name ? 1 : -1))
      return { logs: out }
    },

    /** 读一段日志（默认只回尾部，免得把几十万行的渲染日志整个搬进浏览器）。 */
    async 'logs.read'(a) {
      const id = String((a && a.id) || '')
      if (!ID_RE.test(id)) return { error: '项目 id 非法' }
      const rel = normRel(id, String((a && a.name) || ''))
      if (!relOk(rel) || rel.indexOf('output/logs/') !== 0) return { error: '日志路径非法' }
      const text = await readText(id + '\\' + rel.replace(/\//g, '\\'), '')
      const max = Number((a && a.maxChars) || 200000)
      return {
        name: rel.replace(/^.*\//, ''),
        chars: text.length,
        truncated: text.length > max,
        text: text.length > max ? text.slice(text.length - max) : text,
      }
    },

    /**
     * 全局活动：**现在有没有管线在跑**，以及它的实时日志尾巴。
     *
     * 为什么不复用 logs.list：那是"当前项目的历史日志"，两个毛病 ——
     *   ① 从命令行（manju-headless.py）起的渲染，界面完全不知道；
     *   ② 用户切到别的项目，正在跑的那条日志就看不见了。
     * 用户的诉求原话是"只要管线渲染就需要显示实时日志，他不针对单项目"，
     * 所以这里读的是**全局**的活动记录 `D:\Ai\漫剧\_active.json`（驱动器每次命令都会写/更新它），
     * 再按"结束标记 + 日志文件 mtime"判断存活 —— mtime 会随渲染进度每 30 秒刷新。
     */
    async activity() {
      const act = await readJson('_active.json', null)
      const fsp = await import('node:fs/promises')
      const npath = await import('node:path')
      const now = Date.now()
      const out = { running: false, stale: false, active: null, tail: '', recent: [] }

      if (act && act.rel) {
        const rel = String(act.rel).replace(/\//g, '\\')
        let mtime = 0
        let size = 0
        try {
          const st = await fsp.stat(npath.join(ROOT, rel))
          mtime = st.mtimeMs
          size = st.size
        } catch (e) { /* 文件没了 */ }
        const ended = !!act.endedAt
        // 存活判据：还没写结束标记，且日志在最近 120 秒内被写过
        const live = !ended && mtime > 0 && (now - mtime) < 120000
        out.active = {
          project: String(act.project || ''),
          cmd: String(act.cmd || ''),
          iso: String(act.iso || ''),
          argv: String(act.argv || ''),
          startedAt: Number(act.startedAt || 0),
          endedAt: Number(act.endedAt || 0),
          code: act.code === undefined ? null : Number(act.code),
          rel: String(act.rel || ''),
          mtime: mtime,
          size: size,
          ageMs: mtime ? now - mtime : -1,
        }
        out.running = live
        // 进程被外部杀掉（例如重启桌面端）时不会写 endedAt：明确标成"中断"而不是一直转圈
        out.stale = !ended && !live && mtime >= 0
        if (size > 0) {
          const text = await readText(rel, '')
          const max = Number((arguments[0] && arguments[0].maxChars) || 60000)
          out.tail = text.length > max ? text.slice(text.length - max) : text
        }
      }

      // ── 宿主自己的作业（界面起的管线 / 「渲染」按钮）也必须在"全局活动"里可见 ──
      // 事故（2026-09-23）：原实现只读 _active.json（那是**驱动器**写的），于是界面起的管线
      // 在运行条上完全不可见 —— 用户看到的是"说在跑、日志空白、还点不到停止"。
      let hj = null
      const jkeys = Object.keys(jobs)
      for (let i = 0; i < jkeys.length; i++) {
        const j = jobs[jkeys[i]]
        if (!j.running) continue
        if (!hj || (j.startedAt || 0) > (hj.startedAt || 0)) hj = j
      }
      if (hj) {
        const tailMax = Number((arguments[0] && arguments[0].maxChars) || 60000)
        out.hostJobId = hj.id
        out.hostJob = {
          jobId: hj.id, project: hj.project, kind: hj.kind, mode: hj.mode || '', episode: hj.episode || '',
          stage: jobStageLabel(hj), elapsedSec: jobAgeSec(hj), quietSec: jobQuietSec(hj),
          stalled: !!hj.stalled, logRel: hj.logRel || '',
        }
        // 复用既有的 `active` 形状喂给运行条（不另造一套 UI 渲染）
        out.active = {
          project: hj.project,
          cmd: hj.kind === 'render' ? '渲染' : ('管线 ' + String(hj.mode || '')),
          iso: new Date(hj.startedAt || Date.now()).toISOString(),
          argv: hj.id + (hj.episode ? ' --episode ' + hj.episode : ''),
          startedAt: (hj.startedAt || Date.now()) / 1000,
          endedAt: 0, code: null, rel: hj.logRel || '', mtime: Date.now(),
          size: hj.log.length, ageMs: 0,
        }
        out.running = true
        out.stale = false
        out.tail = hj.log.length > tailMax ? hj.log.slice(hj.log.length - tailMax) : hj.log
      }

      // 跨项目的最近运行记录（下拉里可以直接点开看）
      try {
        const dirs = await listDirEntries(ROOT)
        const rows = []
        for (let i = 0; i < dirs.length; i++) {
          const d = dirs[i]
          if (d.type !== 'directory' || d.name.charAt(0) === '_') continue
          const entries = await listDir(d.name + '\\output\\logs')
          for (let j = 0; j < entries.length; j++) {
            const e = entries[j]
            if (e.type !== 'file' || !/\.log$/i.test(e.name)) continue
            let mt = 0
            try { mt = (await fsp.stat(npath.join(abs(d.name), 'output', 'logs', e.name))).mtimeMs } catch (err) { /* 忽略 */ }
            rows.push({ project: d.name, name: e.name, size: e.size || 0, mtime: mt })
          }
        }
        rows.sort((x, y) => (y.mtime || 0) - (x.mtime || 0))
        out.recent = rows.slice(0, 10)
      } catch (e) { /* 最近记录拿不到不影响实时监视 */ }
      return out
    },
    ...
    // ── 小说工作区（D:\Ai\小说）：作品管理 + 一键做视频 ──

    /** 工作区概况：根目录、是否存在、方案阶段正文上限。set 给了就顺带改目录。 */
    async 'novel.root'(a) {
      if (a && a.set !== undefined) {
        try {
          await setNovelRoot(a.set)
        } catch (e) {
          return { error: String((e && e.message) || e) }
        }
      }
      ...
      return {
        root: root,
        defaultRoot: NOVEL_ROOT_DEFAULT,
        exists: await exists(root),
        planInputMax: PLAN_INPUT_MAX,
      }
    },
    ...
    async 'novel.works'() {
      ...
      if (!(await exists(root))) {
        return { root: root, count: 0, works: [], error: '小说工作区不存在：' + root + '（可在上方改目录）' }
      }
      const dirs = await workDirs()
      const works = []
      for (let i = 0; i < dirs.length; i++) works.push(await workSummary(dirs[i]))
      return { root: root, count: works.length, works: works }
    },

    /** 作品详情：立项书 + 逐章清单。 */
    async 'novel.work'(a) {
      const work = String((a && a.work) || '').trim()
      if (!WORK_RE.test(work) || work.indexOf('..') >= 0) return { error: '作品名非法' }
      if ((await workDirs()).indexOf(work) < 0) return { error: '没有这部作品：' + work }
      return await workDetail(work)
    },

    /** 读作品里的一个文本文件（章 / 设定集 / 全本），有界返回。 */
    async 'novel.read'(a) {
      const work = String((a && a.work) || '').trim()
      if (!WORK_RE.test(work) || work.indexOf('..') >= 0) return { error: '作品名非法' }
      if ((await workDirs()).indexOf(work) < 0) return { error: '没有这部作品：' + work }
      const root = await novelRootPath()
      return await readWorkText(root + '\\' + work, (a && a.rel) || '', (a && a.maxChars) || 20000)
    },

    /**
     * 一键：把小说工作区里的一部作品做成一集漫剧。
     *
     * 一次调用做完这件事：按范围取正文（并如实报告截断）→ 建/复用项目 →
     * 写本集素材与溯源 → 登记集号 → 立刻开跑 AI 一条龙。
     *
     * 「一部小说 = 一个项目」：第二次做第二集时落进同一个项目（workProjects 记住了映射），
     * 集是项目内的概念 —— 这与工作台既有的分集数据模型一致，也避免了"一集一项目"这个坑。
     */
    async 'novel.toVideo'(a) {
      const work = String((a && a.work) || '').trim()
      if (!WORK_RE.test(work) || work.indexOf('..') >= 0) return { error: '作品名非法' }
      if ((await workDirs()).indexOf(work) < 0) return { error: '没有这部作品：' + work }
      const root = await novelRootPath()
      const dir = root + '\\' + work
      const sum = await workSummary(work)
      if (!sum.chapterCount) return { error: '这部作品还没有正文（约定：正文/<卷>/第NNN章_*.md）' }

      const scope = await workScope(dir, a && a.scope)
      if (scope.error) return { error: scope.error }
      if (!scope.text.trim()) return { error: '选定范围里没有正文' }

      const cfg = (await readJson(STUDIO_CFG, null)) || {}
      const known = (cfg.workProjects && cfg.workProjects[work]) || ''
      const id = String((a && a.id) || '').trim() || known || slugId(sum.title)
      if (!ID_RE.test(id)) return { error: '项目 id 只能用字母/数字/下划线/连字符：' + id }
      const isNew = !(await exists(id + '\\project.json'))
      if (!isNew && known && id !== known) {
        return { error: '项目 ' + id + ' 已经是别的小说的项目了（本作品的项目是 ' + known + '）' }
      }

      const title = String((a && a.title) || '').trim() || (isNew ? sum.title : '')
      const style = String((a && a.style) || '').trim() || DEFAULT_STYLE_3D
      const genre = String((a && a.genre) || '').trim() || sum.genre
      const epName = String((a && a.epName) || '').trim() || (scope.label || sum.title)

      const prevMeta = isNew ? {} : await readJson(id + '\\project.json', {})
      const episodes = Object.assign({}, prevMeta.episodes || {})
      // 集号：没指定就自动排下一集（"再做一集"因此仍然是一键，也不会误覆盖已有集）
      let ep = String((a && a.episode) || '').trim()
      if (!ep) ep = nextEpisodeId(episodes)
      if (!/^[A-Za-z0-9_-]{1,20}$/.test(ep)) return { error: '集号只能用字母/数字/下划线/连字符（如 ep01）' }
      const warning = (episodes[ep] && episodes[ep] !== epName)
        ? ('项目里已有 ' + ep + '（' + episodes[ep] + '），本次会用「' + epName + '」覆盖它') : ''
      episodes[ep] = epName
      ...
      if (isNew) {
        await createProjectDir({
          ...
          genre: genre,
          style: style,
          synopsis: sum.logline || sum.hook || '',
          series: work,                     // 系列资产池按作品归集：跨集必须同一张脸
          episodes: episodes,
          origin: { from: 'novel', work: work, dir: dir, at: new Date().toISOString() },
        })
      } else {
        const m = Object.assign({}, prevMeta)
        m.episodes = episodes
        if (style && !m.style) m.style = style
        m.origin = m.origin || { from: 'novel', work: work, dir: dir, at: new Date().toISOString() }
        await writeText(id + '\\project.json', JSON.stringify(m, null, 2))
      }

      // 本集素材单独一份（novel-<集>.md），**不覆盖 novel.md** ——
      // 那是"当前正文"，重跑旧集的方案要靠本集自己的那份。
      await writeText(id + '\\novel-' + ep + '.md', scope.text)
      if (isNew) await writeText(id + '\\novel.md', scope.text)
      // 溯源：这一集来自哪本书的哪些章。改编来源必须可追，否则"这版是哪来的"永远说不清。
      await writeText(id + '\\novel-source-' + ep + '.json', JSON.stringify({
        work: work,
        workDir: dir,
        title: sum.title,
        genre: sum.genre,
        ...
        chars: scope.chars,
        totalChars: scope.totalChars,
        truncated: scope.truncated,
        maxChars: scope.maxChars,
        sourceChars: sum.chars,
        sourceChapters: sum.chapterCount,
        art: sum.art || '',
        createdAt: new Date().toISOString(),
      }, null, 2))

      cfg.workProjects = cfg.workProjects || {}
      cfg.workProjects[work] = id
      await writeText(STUDIO_CFG, JSON.stringify(cfg, null, 2))
      ...
      if (a && a.autoStart === false) return out
      const ps = await handlers.pipelineStart({
        id: id,
        mode: (a && a.mode) || 'ai',
        episode: ep,
        retries: 2,
      ...
      if (ps && ps.error) {
        out.error = ps.error
        out.content = ps.content || null
        return out
      }
      out.jobId = (ps && ps.jobId) || ''
      return out
    },
    ...
    /** 生成独立封面（成品列表的门面）。force=true 时重做一张。 */
    async 'cover.gen'(a) {
      const id = String((a && a.id) || '')
      if (!ID_RE.test(id)) return { error: '项目 id 非法' }
      const meta = await readJson(id + '\\project.json', {})
      const params = Object.assign(defaultParams(), meta.params || {})
      const force = !!(a && a.force)
      if (!force && (await coverOf(id))) return { ok: true, skipped: true, cover: await coverOf(id) }
      const job = { project: id, log: '', stop: false, stages: [] }
      const cst = await ensureComfy(job, params)
      if (!cst.ok) return { ok: false, error: cst.error, log: job.log }
      const r = await genCover(id, params, job)
      return Object.assign({}, r, { log: job.log })
    },
    ...
    async products(a) {
      ...
      // ── 成片（每集一个）──
      // 单独列出来，而不是混在 24 个镜头卡里：成片是"要交付的东西"，
      // 用户问"成片的管理界面在哪"就是因为它们原来沉在镜头列表最底部、面板还是收起的。
      // 集号从 成片-<集>.mp4 解析；没带集号的老命名归一为「全片」。
      const films = clips.filter((c) => c.final).map((c) => {
        const m = /^成片-(.+)\.mp4$/i.exec(c.name)
        return {
          name: c.name,
          episode: m ? m[1] : '',
          label: m ? m[1] : '全片',
      ...
      const epNames = ((await readJson(id + '\\project.json', {})).episodes) || {}
      for (let i = 0; i < films.length; i++) {
        const t = epNames[films[i].episode]
        if (t) films[i].title = String(t)   // 片头卡上的集名，列表里直接用
      }
      return {
    ...
    async pipelineStart(a) {
      ...
      const keys = Object.keys(jobs)
      for (let i = 0; i < keys.length; i++) {
        const run = jobs[keys[i]]
        if (!run.running) continue
        if (a && a.force) {
          // 强制放行：用户已确认那个作业其实不在跑（没有渲染进程、显卡空闲）。
          // 之所以要留这条出口：作业可能卡在某个 await 里收不到停止信号 ——
          // 没有出口的话，用户除了重启 DSH 别无他法（实测踩过）。
          run.log += '\n[强制解除] 用户确认该作业已不在运行，已放行。\n'
          try { if (run.handle) run.handle.terminate() } catch (e) { /* 可能已退出 */ }
          run.running = false
          if (run.exitCode === null) run.exitCode = -1
          run.stalled = true
          await flushJobLog(run)
          continue
        }
        // 挡住的时候必须把"它在干嘛"说清楚：项目、跑了多久、卡在哪个阶段、距上次输出多久。
        // 只说"已有一条管线在运行"等于让用户对着空气猜（原话：什么狗鸡巴，日志又没有）。
        return {
          error: '已有一条管线在运行：' + keys[i] + '（项目 ' + run.project
            + '，已跑 ' + jobAgeSec(run) + ' 秒'
            + (jobStageLabel(run) ? '，' + jobStageLabel(run) : '')
            + '，距上次输出 ' + jobQuietSec(run) + ' 秒）。'
            + '并发跑管线会共用同一张显卡、互相拖慢，所以请先等它结束，或点「停止」。'
            + '如果你确认它其实已经不在跑（没有渲染进程、显卡空闲），点「强制解除」即可继续。',
          busy: {
            jobId: keys[i], project: run.project, stage: jobStageLabel(run),
            elapsedSec: jobAgeSec(run), quietSec: jobQuietSec(run),
            logRel: run.logRel || '',
          },
        }
      }
      ...
      // 集号：给了就整条管线按这一集走（方案取本集素材、渲染清单按集裁、成片带集名）。
      // 与驱动器的 `--episode` 是同一个概念，口径也对齐（字母/数字/下划线/连字符）。
      const episode = String((a && a.episode) || '').trim()
      if (episode && !/^[A-Za-z0-9_-]{1,20}$/.test(episode)) {
        return { error: '集号非法（字母/数字/下划线/连字符，如 ep01）：' + episode }
      }
      seq += 1
      ...
      const job = {
        ...
        // ── 作业健康（看门狗与"它在干嘛"靠这几个字段）──
        startedAt: now0, lastBeat: now0, lastLogLen: 0, logFlushedLen: -1, stalled: false, logRel: '',
      }
      jobs[jobId] = job
      runPipeline(job).catch((e) => {
        ...
        // 无论成功、失败还是抛异常，都保证：日志落盘 + 作业被清理。
        // （原实现只在"正常结束"时落盘，于是卡死的那次什么痕迹都没留下。）
        await flushJobLog(job)
        pruneJobs()
      })
      return { ok: true, jobId }
    ...
    /** 现在有哪些作业（在跑的在前面）。界面靠它认领"不是我发起的那个作业"。 */
    async 'jobs.list'() {
      const keys = Object.keys(jobs)
      const out = []
      for (let i = 0; i < keys.length; i++) {
        ...
        out.push({
          ...
          elapsedSec: jobAgeSec(job),
          quietSec: jobQuietSec(job),
          stages: job.stages.map((s) => ({ key: s.key, name: s.name, state: s.state, note: s.note })),
          logRel: job.logRel || '',
        })
      }
      out.sort((x, y) => (y.running ? 1 : 0) - (x.running ? 1 : 0) || y.elapsedSec - x.elapsedSec)
      return { jobs: out, running: out.filter((j) => j.running).length }
    },

    async pipelinePoll(a) {
      const wantId = String((a && a.jobId) || '')
      let job = jobs[wantId]
      if (!job && wantId) return { error: '任务不存在', gone: true }
      if (!job) {
        // 不带 jobId ＝ "现在有什么在跑？" —— 换页面 / 重启渲染器之后，
        // 界面靠它认领在跑的作业（否则运行条永远空白，用户看不到日志也点不到停止）。
        const keys = Object.keys(jobs)
        for (let i = keys.length - 1; i >= 0; i--) {
          if (jobs[keys[i]].running) { job = jobs[keys[i]]; break }
        }
        if (!job) return { running: false, jobs: [] }
      }
      const lines = jobLines(job)
      return {
        jobId: job.id,
    ...
    async pipelineStop(a) {
      ...
      for (let i = 0; i < keys.length; i++) {
        const job = jobs[keys[i]]
        if (wantId ? job.id === wantId : job.running) targets.push(job)
      }
      if (!targets.length) return { error: '没有在跑的作业（或指定的作业不存在）' }
      for (let i = 0; i < targets.length; i++) {
        const job = targets[i]
        job.stop = true
        try { if (job.handle) job.handle.terminate() } catch (e) { /* 进程可能已退出 */ }
        // **立即放行**：卡在某个 await 里的作业永远收不到 stop 信号，
        // 只设 stop=true 的话界面会继续被"已有一条管线在运行"挡住（实测踩过）。
        job.running = false
        if (job.exitCode === null) job.exitCode = -2
        job.log += '\n[已停止] 用户点了停止，作业已放行。\n'
        // **不 await**：落盘可能因为文件系统/服务抽风而变慢，而"停止"必须立刻返回 ——
        // 用户点停止是为了脱困，不是为了等一次磁盘写。
        flushJobLog(job).catch(() => { /* 落盘失败不影响停止 */ })
      }
      return { ok: true, stopped: targets.map((j) => j.id) }
    },
  ...
  function readBody(req) {
    return new Promise((resolve) => {
      let data = ''
      let dead = false
      // **必须让 Node 按 utf8 解码**：原来 `data += c` 对每个 Buffer 分块**各自**解码，
      // 一个 3 字节汉字被 TCP 分块切开时，两半各解成 U+FFFD → 粘贴的小说/剧本里静默出现乱码（且已落盘）。
      // setEncoding 会把跨块边界的半个字符留住，等下一块拼上。
      // setEncoding 会把跨块边界的半个字符留住，等下一块拼上。
      // 加 typeof 守卫：测试桩里的假 req 没有这个方法，直接调会把整套测试打挂。
  ...
  /**
   * 媒体流服务的**公共实现**：ETag/304 + Range/206 + HEAD + 416。
   *
   * 为什么要抽出来：项目产物与小说封面是**两个不同的根**
   * （项目根 `D:\Ai\漫剧\<项目>`、小说工作区 `D:\Ai\小说\<作品>`），
   * 但媒体语义必须逐字一致 —— 复制一份迟早会漂移，而这段里的每一行都对应过一次事故：
   * Cache-Control 用 no-store → 封面一闪一闪；错误响应挂死 → 界面卡住。
   * 调用方只负责"在给定根之下把路径校验干净"，本函数负责"把文件安全地送出去"。
   */
  async function serveMedia(req, res, opts) {
    const fileAbs = String(opts.fileAbs || '')
    const clean = String(opts.clean || '')
    const diag = opts.diag || {}
    const target = await fs.resolve(fileAbs)
    const st = await fs.stat(target)
    if (!st || st.type !== 'file') {
      res.writeHead(404, { 'X-Manju-Diag': diagH(JSON.stringify(diag)) })
      res.end('not found')
      return
    }
    ...
    // ── 缓存语义（这一条直接决定封面闪不闪）──
    // 旧实现一律 `Cache-Control: no-store`：浏览器每次都**重新下载 + 重新解码**，
    // 于是任何一次重绘/重挂载都让封面"一闪一闪"。
    // 改成强校验器 + no-cache（每次条件请求，内容没变就 304）：
    //   * 内容一致 → 复用已解码的图，不闪、不重传；
    //   * 内容变了（重渲过的镜头、重抽的缩略图）→ 立刻 200 新内容，绝不发陈旧数据。
    // 用 node:fs 直接 stat 取 mtimeMs：宿主 fs 服务的 stat 形状不保证带它，
    // 而 ETag 的强弱全看这个字段。
    const nodeFsp = await import('node:fs/promises')
    let size = 0
    let mtime = 0
    try {
      const nst = await nodeFsp.stat(fileAbs)
      size = nst.size
      mtime = nst.mtimeMs
    } catch (e) {
      res.writeHead(404, { 'X-Manju-Diag': diagH(JSON.stringify(diag)) })
    ...
    const etag = '"' + size.toString(16) + '-' + Math.round(mtime).toString(16) + '"'
    res.setHeader('Content-Type', ct)
    res.setHeader('ETag', etag)
    res.setHeader('Last-Modified', new Date(mtime).toUTCString())
    res.setHeader('Accept-Ranges', 'bytes')
    res.setHeader('Cache-Control', 'private, no-cache')
    const inm = String((req.headers && req.headers['if-none-match']) || '')
    const ims = String((req.headers && req.headers['if-modified-since']) || '')
    const notMod = (inm && inm.indexOf(etag) >= 0)
      || (!inm && ims && Date.parse(ims) >= Math.floor(mtime / 1000) * 1000)
    if (notMod) {
      res.writeHead(304)
      res.end()
      return
    ...
    // ── Range ──
    // 不支持 206 的话，<video> 拖进度条得先把整个文件下完（成片 37 MB）；
    // 支持了就能边下边播、任意 seek。请求非法范围按规范回 416。
    ...
    if (mm && size > 0) {
      if (mm[1] !== '') {
        ...
        end = mm[2] !== '' ? parseInt(mm[2], 10) : size - 1
      } else if (mm[2] !== '') {
        start = Math.max(0, size - parseInt(mm[2], 10))
        end = size - 1
      }
    ...
    const stream = nodeFs.createReadStream(fileAbs, { start: start, end: end })
    if (partial) {
      res.writeHead(206, {
    ...
    stream.on('error', () => { try { res.end() } catch (e2) { /* 已响应 */ } })
    stream.pipe(res)
  }
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        diag.clean = clean
        if (!relOk(clean)) {
          res.writeHead(400, { 'X-Manju-Diag': diagH(JSON.stringify(diag)) })
        ...
        if (!diag.prefixOk) {
          res.writeHead(403, { 'X-Manju-Diag': diagH(JSON.stringify(diag)) })
          res.end('outside project')
          return
        }
        await serveMedia(req, res, { fileAbs: fileAbs, clean: clean, diag: diag })
      } catch (e) {
        try {
          res.writeHead(500, { 'X-Manju-Diag': diagH(JSON.stringify(diag) + '|err=' + String(e && e.message)) })
          res.end('error')
        } catch (e2) { /* 已响应 */ }
      }
    },
  ...
  /**
   * 小说工作区的媒体接口：GET /manju-novel-file?w=<作品目录名>&r=<作品内相对路径>。
   *
   * 为什么要单独一条路由而不是给 /manju-file 加个 root 参数：
   * 参数一旦能指定任意根目录，路径前缀校验就形同虚设（那就是任意文件读取）。
   * 两个根各自一条路由，各自校验，语义共用 serveMedia。
   */
  ctx.effect(() => ctx.webServer.register({
    kind: 'prefix',
    path: NOVEL_FILE_PATH,
    handler: async (req, res) => {
      const diag = { work: '', clean: '', abs: '', prefixOk: false }
      try {
        const url = String((req && req.url) || '')
        const qi = url.indexOf('?')
        const parts = (qi >= 0 ? url.slice(qi + 1) : '').split('&')
        ...
        for (let i = 0; i < parts.length; i++) {
          const kv = parts[i].split('=')
          if (kv[0] === 'w') work = decodeURIComponent(kv.slice(1).join('='))
          else if (kv[0] === 'r') rel = decodeURIComponent(kv.slice(1).join('='))
        }
        diag.work = work
        if (!WORK_RE.test(work) || work.indexOf('..') >= 0) { res.writeHead(400); res.end('bad work id'); return }
        const clean = String(rel).replace(/\\/g, '/').replace(/^\/+/, '')
        diag.clean = clean
        ...
        // novelRootPath 是异步的（配置可能要先读盘）—— 漏掉 await 会让路径变成
        // "[object Promise]\\<作品>"，症状是封面一律 404 而其它功能全都正常。
        const rootAbs = (await novelRootPath()) + '\\' + work
        const fileAbs = rootAbs + '\\' + clean.replace(/\//g, '\\')
        diag.abs = fileAbs
        diag.prefixOk = fileAbs.toLowerCase().indexOf((rootAbs + '\\').toLowerCase()) === 0
        if (!diag.prefixOk) {
          res.writeHead(403, { 'X-Manju-Diag': diagH(JSON.stringify(diag)) })
          res.end('outside work')
          return
        }
        await serveMedia(req, res, { fileAbs: fileAbs, clean: clean, diag: diag })
      } catch (e) {
